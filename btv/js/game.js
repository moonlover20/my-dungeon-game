...
const ENEMY_TYPES={
  ...
  goblin_bomber  :{name:"블페러", r:15, hp:22, spd:114, dmg:8,  color:"#9aa83f", xp:11, ai:"chase", explode:true, label:"블페러"},
...
const ACT_POOLS=[
  ...
  { normal:["gwangcheon_gim","reura","namu","pobear"], elite:["pobear","namu"] },
  ...
  if(kind==='boss'){
    ...
    if(kind==='midboss'){
      if(act>=2){
        ...
        eb.elite=true; eb.midboss=true; eb.label='양갱'; eb.atkT=1.8; eb.atkN=0;
        banner("중간보스 · 양갱","말랑한데 단단하다",1800);
        if(typeof sfx!=='undefined') sfx.boss();
        showEntrance("⚠️ 중간보스 등장","양갱","연양갱 — 흑임자의 분노");
      } else {
...
function updateBoss(dt){
  ...
  if(b.attackT<=0){
    ...
    if(b.pattern==='summon'){
      b.atkN=(b.atkN||0)+1;
      const phase=b.atkN%5;
      if(phase===0){
        // ① 조준 5갈래 음식 부채 (소환 제거)
        const pa=Math.atan2(player.y-b.y,player.x-b.x);
        for(let i=-2;i<=2;i++) throwFood(b,pa+i*0.15,205,10);
      } else if(phase===1){
        // ② 조준 부채꼴
        const pa=Math.atan2(player.y-b.y,player.x-b.x), k=b.enraged?9:7;
        for(let i=0;i<k;i++) throwFood(b,pa+(i-(k-1)/2)*0.16,205,10);
      } else if(phase===2){
        // ③ 2갈래 회전 나선
        ...
        // ④ 이중 사방 링
        const k=b.enraged?16:13;
        for(let i=0;i<k;i++){ throwFood(b, i/k*TAU, 185, 9); throwFood(b, i/k*TAU+Math.PI/k, 135, 9); }
      } else {
        // ⑤ 음식 폭격 (낙하·증가)
...
function update(dt){
  ...
  for(const e of enemies){
    ...
    if((e.stunT=(e.stunT||0))>0){ e.stunT-=dt; }
      ...
      if(e.type==='goblin_bomber'){ if(d<175){ e.x+=Math.cos(a)*e.spd*0.95*dt; e.y+=Math.sin(a)*e.spd*0.95*dt; if(!e._fuse){ e._fuse=true; burst(e.x,e.y,'#ff4d4d',6,150); if(typeof beep==='function')beep(90,0.05,'square',0.05); } } else { e._fuse=false; } }
      // 나무 피격 방어력 버프 타이머 감소
      ...
      e.faceAng=a;
      if(!e.enr && e.hp<e.maxhp*0.4){ e.enr=true; e.spd*=1.3; banner('양갱 격노','말랑말랑 분노!',900); if(typeof sfx!=='undefined'&&sfx.boss)sfx.boss(); }
      e.atkT=(e.atkT==null?1.6:e.atkT)-dt;
      if(e.atkT<=0){
        e.atkN=(e.atkN||0)+1; const pa=a, en=e.enr, ph=e.atkN%3;
        if(ph===0){ const k=en?9:7; for(let i=0;i<k;i++){ const a2=pa+(i-(k-1)/2)*0.16; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*250,vy:Math.sin(a2)*250,r:8,dmg:10,life:3.4,srcName:'양갱'}); } }
        else if(ph===1){ const k=en?16:12; for(let i=0;i<k;i++){ const a2=i/k*TAU+e.atkN*0.4; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*200,vy:Math.sin(a2)*200,r:8,dmg:9,life:3.6,srcName:'양갱'}); } }
        else { const base=pa-0.6; for(let i=0;i<(en?10:7);i++){ const a2=base+i*0.13; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*230,vy:Math.sin(a2)*230,r:7,dmg:9,life:3.4,srcName:'양갱'}); } }
...
function renderMap(){
  ...
  mapData.edges.forEach(e=>{
    ...
    s+='<line x1="'+a.x+'" y1="'+a.y+'" x2="'+b.x+'" y2="'+b.y+'" stroke="'+(active?'#38e8ff':'#3a2d5c')+'" stroke-width="'+(active?3:2)+'"/>';
  });
...
function onCombatCleared(){
  ...
  setTimeout(()=>{
    ...
    if(t==='midboss') offerRelics(3,'중간보스 보상',(act>=2?'양갱을 쓰러뜨린 보상이다':'혜철이를 쓰러뜨린 보상이다'), finishNode);
...
const LEVEL_PERKS=[
  // ===== 일반 Common =====
  {g:'common',icon:'⚔️',name:'공격 특화',desc:'공격력 +10%',apply:p=>{p.dmgMul+=0.10;}},
...
const SMALL_REWARDS=[
  ...
  {icon:'⚔️',name:'무기 손질',desc:'공격력 +6%',apply:()=>{player.dmgMul*=1.06;banner('공격력 +6%','',1000);}},
  {icon:'⚡',name:'방아쇠 정비',desc:'발사 속도 +5%',apply:()=>{player.fireMul*=0.95;banner('발사 속도 +5%','',1000);}},
  ...
  {icon:'🎯',name:'조준경',desc:'치명타 확률 +5%',apply:()=>{player.critChance=Math.min(1,player.critChance+0.05);banner('치명타 +5%','',1000);}},
  {icon:'💥',name:'탄 개조',desc:'투사체 크기 +8%',apply:()=>{player.bulletSize*=1.08;banner('투사체 크기 +8%','',1000);}},
  ...
  {icon:'🔋',name:'추진제',desc:'투사체 속도 +15%',apply:()=>{player.bulletSpeedMul*=1.15;banner('투사체 속도 +15%','',1000);}},
  {icon:'🧲',name:'자력 코일',desc:'골드 획득 +10%',apply:()=>{player.goldMul+=0.1;banner('골드 획득 +10%','',1000);}},
  {icon:'🩸',name:'흡혈 코팅',desc:'흡혈 확률 +4%',apply:()=>{player.lifesteal+=0.04;banner('흡혈 +4%','',1000);}},
...
const SPRITES={
  ...
  yanggaeng:(r,e)=>{ if(yangReady){const ih=YANG_SPRITE.naturalHeight||1;const sh=r*1.95;const sw=sh*(YANG_SPRITE.naturalWidth/ih);ctx.drawImage(YANG_SPRITE,-sw/2,-sh/2,sw,sh);return;} // 양갱 (중간보스)
    const en=e&&e.enr;
    ctx.fillStyle='#1a1208'; ctx.fillRect(-r*1.2,-r*0.5,r*2.4,r*1.0);   // 검정 포장
    ctx.fillStyle=en?'#e0913f':'#b5662a'; ctx.fillRect(-r*0.85,-r*0.42,r*1.7,r*0.84); // 본체
    ctx.strokeStyle='#3a2010'; ctx.lineWidth=Math.max(2,r*0.08); ctx.strokeRect(-r*0.85,-r*0.42,r*1.7,r*0.84);
    ctx.fillStyle='#f0d8a0'; ctx.fillRect(-r*0.3,-r*0.3,r*0.6,r*0.6); // 라벨
    ctx.fillStyle='#7a1f1f'; ctx.font='bold '+Math.round(r*0.52)+'px Courier New'; ctx.textAlign='center'; ctx.fillText('갱',0,r*0.2); ctx.textAlign='left';
...
function drawEliteIntro(){
  ...
  if(E.banner>0){
    const col='#c46bff', bw=470,bh=74,bx=(W-bw)/2, by=62-(1-E.banner)*72;
    ...
    ctx.fillStyle=col; ctx.font='bold 14px sans-serif'; ctx.fillText('⚠ 자잘자', W/2, by+25);
    ctx.fillStyle='#fff'; ctx.font='bold 19px sans-serif'; ctx.fillText('"나는야 너무 멋진 장수풍뎅이"', W/2, by+52);
    ctx.restore();
  }
}
function spawnFirePillar(x,y){ hazards.push({x:clamp(x,34,W-34),y:clamp(y,90,H-50),t:0,warnT:0.85,liveT:0.45,r:38,hit:false,dmg:13}); }
...
function drawEliteBar(e){
  ...
  ctx.fillText('⚔️ 자잘자', W/2, by-8); ctx.textAlign='left';
  ctx.restore();
}
