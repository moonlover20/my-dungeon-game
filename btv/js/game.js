...
const MUSIC = {
  ...
  files:{
    ...
    act2_battle_elite:   'btv/assets/music/act2_battle_elite.mp3',
    act3_battle_normal:  'btv/assets/music/act3_battle_normal.mp3',
    act3_battle_elite:   'btv/assets/music/act3_battle_elite.mp3',
    act3_midboss:        'btv/assets/music/act3_midboss.mp3',
    act3_final_boss:     'btv/assets/music/act3_final_boss.mp3'
  },
  tightLoops:{
...
const BATTLE_BGM = {
  ...
  2: { elite:'act2_battle_elite' },
  3: { normal:'act3_battle_normal', elite:'act3_battle_elite' }
  // 정의가 없는 role(막2 일반/중보/보스 등)은 아래 기존 폴백 로직을 그대로 사용.
};
// 전용 파일이 없거나 로드 실패 시 대체할 트랙(전용곡을 넣으면 files 경로만 채우면 됨)
const BGM_FALLBACK = { act1_battle_normal:'act1', act1_battle_elite:'act1', act1_midboss:'midboss', act1_finalboss:'boss', act2_battle_elite:'act2', act3_battle_normal:'bgm_act3', act3_battle_elite:'bgm_act3', act3_midboss:'act2Midboss', act3_final_boss:'finalBoss' };
...
function desiredMusicKey(){
  ...
  if(roomIsMidboss){
    ...
    if(mb&&mb.type==='onster') return 'act3_midboss';
  }
...
function kiterRush(e,a,d,dt){
  // 안티-터틀: 후반 카이터가 플레이어의 거리벌리기에 들이대 근접 스윕. 러시 중이면 true(일반 AI 스킵)
  if((e._rushCd||0)>0) e._rushCd-=dt;
  if(!e._rush){
    if(d>340) e._turtleT=(e._turtleT||0)+dt; else e._turtleT=Math.max(0,(e._turtleT||0)-dt*1.6);
    if((e._turtleT||0)>=4.5 && (e._rushCd||0)<=0){
      e._rush='wind'; e._rushT=0.5; e._turtleT=0; e.faceAng=a;
      banner('⚡ 들이댄다!','거리를 좁힌다',650); burst(e.x,e.y,e.color,12,180); if(typeof beep==='function')beep(200,0.1,'square',0.05);
      return true;
    }
    return false;
  }
  if(e._rush==='wind'){
    e._rushT-=dt; e.faceAng=a; e._rushAx=Math.cos(a); e._rushAy=Math.sin(a);
    if(e._rushT<=0){ e._rush='dash'; e._rushT=0.6; if(typeof sfx!=='undefined'&&sfx.enemyDash) sfx.enemyDash(); }
    return true;
  }
  if(e._rush==='dash'){
    e._rushT-=dt;
    e.x=clamp(e.x+e._rushAx*e.spd*4.2*dt,e.r,W-e.r);
    e.y=clamp(e.y+e._rushAy*e.spd*4.2*dt,e.r,H-e.r);
    e.faceAng=Math.atan2(e._rushAy,e._rushAx);
    const wall=e.x<=e.r||e.x>=W-e.r||e.y<=e.r||e.y>=H-e.r;
    if(d<e.r+player.r+34 || e._rushT<=0 || wall){
      e._rush='sweep'; e._rushT=0.45;
      warnAoE(e.x,e.y,rand(94,106),0.35,0.4,Math.max(10,Math.round((e.dmg||10)*1.4)),(e.name||e.label)+' 근접 스윕',e.color);
      burst(e.x,e.y,'#ffd34d',16,220); screenShake=Math.max(screenShake||0,5); if(typeof beep==='function')beep(120,0.14,'sawtooth',0.06);
    }
    return true;
  }
  if(e._rush==='sweep'){
    e._rushT-=dt;
    if(e._rushT<=0){ e._rush=null; e._rushCd=rand(6.5,8.5); e.stunT=Math.max(e.stunT||0,0.35); }
    return true;
  }
  return false;
}
...
function bagjeinDoRewind(e){
  // === 박제인간 시그니처: 되감기 — 지나간 탄을 역재생시켜 보스 쪽으로 빨아들임 ===
  banner('⏪ 되감기','지나간 음이 되돌아온다',1100);
  if(typeof sfx!=='undefined'&&sfx.enemyGlitch) sfx.enemyGlitch();
  screenShake=Math.max(screenShake||0,12); if(typeof beep==='function')beep(440,0.08,'square',0.04);
  // 1) 되감을 음(탄막)을 먼저 크게 흩뿌린다 (속도 낮춰 화면에 머물게)
  const k=28; for(let i=0;i<k;i++){ const a2=i/k*TAU+(e.spinAng||0); eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*190,vy:Math.sin(a2)*190,r:8,dmg:14,life:6.5,srcName:'박제인간'}); }
...
function spawnAct3BeamSweep(e){
  ...
  hazards.push({kind:'beamSweep',x:e.x,y:e.y,ang:dir-spin*0.9,rot:spin,range:e.range||620,width:20,warnT:0.90,liveT:2.05,t:0,dmg:Math.max(8,Math.round((e.dmg||16)*0.72)),srcName:e.name||e.label||'노잭',seed:rand(0,TAU)});
...
function act3TruckSignalRing(e){
  ...
  for(let i=0;i<k;i++){ const a2=off+i/k*TAU; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*sp,vy:Math.sin(a2)*sp,r:8,dmg:Math.max(12,Math.round((e.dmg||16)*0.85)),life:3.6,srcName:(e.name||e.label||'노잭'),col:e.color}); }
...
function act3TruckCableBeams(e){
  ...
  for(let k=0;k<2;k++){ kijoLaserWarns.push({x:e.x,y:e.y,ang:base+k*Math.PI/2,width:18,range:780,t:0,warn:0.72,color:'#ff4dd2',fired:false,sniper:true,dmg:Math.max(8,Math.round((e.dmg||16)*0.68)),srcName:(e.name||e.label||'노잭')+' 케이블'}); }
...
function runAct3TruckPattern(e,pat){
  ...
  else if(pat==='flood') act3TruckCommentFlood(e);
  else if(pat==='spinbeam') act3TruckSpinBeam(e);
  else if(pat==='claim') act3TruckClaim(e);
  else if(pat==='newsWall') act3TruckNewsWall(e);
  else if(pat==='megabeam') act3TruckMegaBeam(e);
...
function updateSet3(b,dt){
  ...
  if((b._grabLockT||0)>0){ b._grabLockT-=dt; const ga=Math.atan2(player.y-b.y,player.x-b.x); b.x+=Math.cos(ga)*b.spd*1.5*dt; b.y+=Math.sin(ga)*b.spd*1.0*dt; b._grabPullT=(b._grabPullT||0)-dt; if(b._grabPullT<=0){ b._grabPullT=0.7; if((player.dodging||0)<=0){ const pa=Math.atan2(b.y-player.y,b.x-player.x); player.x=clamp(player.x+Math.cos(pa)*90,player.r,W-player.r); player.y=clamp(player.y+Math.sin(pa)*90,player.r,H-player.r); burst(player.x,player.y,'#ff4dd2',8,150); } if(dist2(b.x,b.y,player.x,player.y)<(b.r+player.r+30)**2) hurtPlayer(10,'현진 그랩'); } }
  ...
  // ── 3막 최종보스 패턴 (셔플백 + 연속 버스트) ──
  b.a3T=(b.a3T==null?5:b.a3T)-dt;
  if(b.a3T<=0){
    if((b._s3Rep||0)>0){
      b._s3Rep--; runSet3Pat(b,b._s3Pat);
      b.a3T = b._s3Rep>0 ? a3Jit(1.5,0.1) : a3Jit(ph===3?5.6:5.3,0.12);
    } else {
      const pool=ph===1?SET3_PATS_P1:ph===2?SET3_PATS_P2:SET3_PATS_P3;
      const bag=ph===1?'_s3Bag1':ph===2?'_s3Bag2':'_s3Bag3';
      const p=pool[nextFromBag(b,bag,pool.length)];
      b._s3Pat=p; runSet3Pat(b,p);
      if(p==='cone'||p==='crossslash'){ b._s3Rep=2+(Math.random()<0.4?1:0); b.a3T=a3Jit(1.5,0.1); }
      else b.a3T=a3Jit(ph===3?5.8:5.5,0.12);
    }
  }
  b.attackT-=dt;
  if(b.attackT<=0){
    ...
    if(ph===1){
      ...
      else if(b.patI%3===0){ const k=18; for(let i=0;i<k;i++){ const aa=b.angle+i/k*TAU; eBullets.push({x:b.x,y:b.y,vx:Math.cos(aa)*205,vy:Math.sin(aa)*205,r:8,dmg:15,life:3.6,srcName:'케케로로 에너지구'}); } }
      else { const pa=Math.atan2(player.y-b.y,player.x-b.x); for(let i=-2;i<=2;i++) eBullets.push({x:b.x,y:b.y,vx:Math.cos(pa+i*0.22)*255,vy:Math.sin(pa+i*0.22)*255,r:8,dmg:16,life:3.4,srcName:'케케로로'}); }
      b.attackT=1.15;
    }
    sfx.shoot();
  }
...
let onsterCross=null, onsterBreath=null, set3Half=null, set3Reaper=null;   // 신규: 교차빔/깊은숨결/반반시간/검은마법
function clearA3Systems(){ a3veil=null; a3seq=null; a3strike=null; a3tether=null; set3Poll=null; set3Ring=null; set3Ad=null; onsterCross=null; onsterBreath=null; set3Half=null; set3Reaper=null; if(typeof player==='object'&&player) player._adBlockT=0; }
...
// 현진 분신 — 진짜(번검 본체)만 선명, 가짜 3은 흐릿. 가짜를 치면 반격탄
function a3Decoys(b){
  ...
  for(let i=0;i<4;i++){ if(i===realIdx) continue;
    enemies.push({type:'decoy',sprite:b.sprite,name:'현진',label:'현진',x:slots[i].x,y:slots[i].y,r:b.r||60,hp:999999,maxhp:999999,_lastHp:999999,spd:0,_spd0:0,dmg:0,touchDmg:0,bodyDmg:0,color:b.color,xp:0,ai:'decoy',life:6,wob:rand(0,TAU),_glitchClone:true,summoned:true,noKillScore:true,noReward:true,hitT:0,coolT:0});
  }
  banner('👥 현진 · 진짜만 선명하다','가짜를 치면 반격탄',1400); beep(120,0.2,'sawtooth',0.05);
}
// 밀어내기 + 링 탄막
function a3PushRing(b,dmg){
  ...
  const k=18; for(let i=0;i<k;i++){ const a2=i/k*TAU; eBullets.push({x:b.x,y:b.y,vx:Math.cos(a2)*215,vy:Math.sin(a2)*215,r:8,dmg:dmg||16,life:3.4,srcName:(b.label||'충격')}); }
...
// ===== ACT3 v2 신규 패턴 라이브러리 (엘리트·중보·최종보스 리워크 / safe·veil·순서기억 폐기) =====
function a3Jit(base,amp){ return base*(1+rand(-(amp||0.1),(amp||0.1))); }
function a3Alive(b){ return state==='play' && b && (b===boss || enemies.includes(b)); }
...
function act3TruckCommentFlood(e){
  ...
  edges.forEach((pt,ei)=>{ for(let i=0;i<5;i++) setTimeout(()=>{ if(!enemies.includes(e))return; const pa=Math.atan2(player.y-pt[1],player.x-pt[0]); eBullets.push({x:pt[0],y:pt[1],vx:Math.cos(pa)*210,vy:Math.sin(pa)*210,r:8,dmg,life:3.6,home:0.6,srcName:e.name||e.label,col:'#58d8ff'}); }, ei*120+i*150); });
  if(typeof beep==='function')beep(360,0.06,'square',0.04);
}
function act3TruckSpinBeam(e){
  // 생방송 송출빔: 보스 중심 양방향 회전 빔 2줄
  if(sfx.enemyCore) sfx.enemyCore();
  const dir=Math.atan2(player.y-e.y,player.x-e.x), spin=(Math.random()<0.5?-1:1)*0.6, dmg=Math.max(13,Math.round((e.dmg||16)*0.8));
  for(let s=0;s<2;s++) hazards.push({kind:'beamSweep',x:e.x,y:e.y,ang:dir+s*Math.PI-spin*0.8,rot:spin,range:e.range||640,width:22,warnT:0.85,liveT:2.2,t:0,dmg,srcName:(e.name||e.label||'노잭')+' 송출빔',seed:rand(0,TAU),hitCd:0});
  burst(e.x,e.y,'#58d8ff',14,170); banner('📡 생방송 송출','회전 빔 — 돌파 타이밍!',700);
}

// ── 중보 온스터 전용 사슬 패턴 ──
const ONSTER_PATS_P1=['grid','chase','whip','bind','poison','anchor','maze','bindexec','counterwhip','reel','crosslaser','deepbreath'];
const ONSTER_PATS_P2=['grid','chase','whip','bind','poison','burst','anchor','maze','broken','cage','bindexec','counterwhip','reel','crosslaser','deepbreath'];
function runOnsterPat(e,p){
  if(p==='grid') onsterChainGrid(e);
  else if(p==='chase') onsterBindChase(e);
  else if(p==='whip') onsterWhipSpin(e);
  else if(p==='bind') a3BindRing(e,e.awakened?5:4,'사슬');
  else if(p==='poison') a3PoisonTrail(e,5,0.5,e.awakened?13:11,'#9d7bff');
  else if(p==='tether') onsterTether(e);
  else if(p==='anchor') onsterChainAnchor(e);
  else if(p==='maze') onsterChainMaze(e);
  else if(p==='broken') onsterBrokenChains(e);
  else if(p==='cage') onsterChainCage(e);
  else if(p==='bindexec') onsterBindExecute(e);
  else if(p==='counterwhip') onsterCounterWhip(e);
  else if(p==='reel') onsterReelToMinions(e);
  else if(p==='crosslaser') onsterCrossLaser(e);
  else if(p==='deepbreath') onsterDeepBreath(e);
  else onsterChainBurst(e);
}
function onsterChainGrid(e){
  // 사슬 격자: 가로 2 + 세로 2 사슬 빔을 경고 후 발사, 격자 틈으로 빠져야 함
  if(sfx.enemyChain) sfx.enemyChain();
  banner('⛓ 사슬 격자','격자 틈으로 빠져라',850);
  const dmg=e.awakened?20:16, col=e.awakened?'#ff4dd2':'#8d72ff', warn=e.awakened?0.6:0.74;
  [H*0.34,H*0.62].forEach(y=>kijoLaserWarns.push({x:0,y,ang:0,width:18,range:W+40,t:0,warn,color:col,fired:false,sniper:true,dmg,srcName:'사슬 격자'}));
  [W*0.33,W*0.66].forEach(x=>kijoLaserWarns.push({x,y:0,ang:Math.PI/2,width:18,range:H+40,t:0,warn,color:col,fired:false,sniper:true,dmg,srcName:'사슬 격자'}));
}
function onsterBindChase(e){
  // 구속 추격 사슬: 플레이어를 시차로 따라오는 사슬 마커가 순차 내려찍음
  player.slowDebuffT=Math.max(player.slowDebuffT||0,1.6);
  if(sfx.enemyChain) sfx.enemyChain();
  banner('🪝 구속 사슬','연속 내려찍기 — 계속 움직여라',1000);
  const n=e.awakened?4:3, dmg=e.awakened?26:22;
  for(let i=0;i<n;i++) setTimeout(()=>{ if(!a3Alive(e))return; hazards.push({x:clamp(player.x,34,W-34),y:clamp(player.y,90,H-50),t:0,warnT:0.5,liveT:0.4,r:96,hit:false,dmg,srcName:'구속 사슬',col:'#8d72ff'}); if(typeof beep==='function')beep(120,0.1,'sawtooth',0.05); }, 480+i*420);
}
function onsterWhipSpin(e){
  // 회전 사슬 채찍: 보스 중심 2갈래 회전 바
  if(sfx.enemyChain) sfx.enemyChain();
  const dmg=e.awakened?18:15, dir=(Math.random()<0.5?1:-1);
  for(let s=0;s<2;s++) hazards.push({kind:'spinbar',owner:e,x:e.x,y:e.y,len:e.awakened?300:250,wid:24,ang:rand(0,TAU)+s*Math.PI,rot:dir*(e.awakened?2.6:2.1),t:0,warnT:0.65,liveT:e.awakened?4.2:3.6,dmg,hitCd:0,srcName:'사슬 채찍',col:e.color});
  banner('⛓ 사슬 채찍','회전 사슬을 피해라',750);
}
function onsterChainBurst(e){
  // [각성] 사슬 파편 폭발: 회전 오프셋 전방위 링
  const k=20, off=(e._cbN=(e._cbN||0)+1)*0.3, dmg=16;
  for(let i=0;i<k;i++){ const a=off+i/k*TAU; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*210,vy:Math.sin(a)*210,r:8,dmg,life:3.4,srcName:'사슬 파편',col:'#ff4dd2'}); }
  burst(e.x,e.y,'#ff4dd2',14,170);
}
...
function runSet3Pat(b,p){
  ...
  else if(p==='cross') a3CrossAoE(b,Math.max(W,H),60,1.0,20);
  else if(p==='spin') a3SpinBar(b,(b.setPhase||1)===2?260:230,16,3.8);
  else if(p==='push') a3PushRing(b,16);
  else if(p==='vine') a3VineSlam(b,'현진 그랩');
  ...
  // P2 번검 (검기·분신)
  else if(p==='breath') a3SweepBreath(b,Math.random()<0.5?-1:1,18);
  else if(p==='brand') a3Brand(b,1.6,96,24,'번검 낙인');
  else if(p==='decoy') a3Decoys(b);
  else if(p==='crossslash') set3CrossSlash(b);
  else if(p==='cloneslash') set3CloneSlash(b);
...
function set3QuakeChain(b){
  // 지면강타 충격 연쇄: 보스→플레이어 직선상으로 확장 충격파 3연
  if(sfx.enemyDash) sfx.enemyDash();
  banner('💥 지면 강타','충격 연쇄 — 옆으로 피해라',900);
  const pa=Math.atan2(player.y-b.y,player.x-b.x);
  for(let i=0;i<3;i++) setTimeout(()=>{ if(!a3Alive(b))return; const px=clamp(b.x+Math.cos(pa)*(150+i*150),60,W-60), py=clamp(b.y+Math.sin(pa)*(150+i*150),100,H-70); intentShockwave(px,py,110,20,'현진 지면강타'); screenShake=Math.max(screenShake||0,8); }, i*250);
}
// 번검(P2) 신규
function set3CrossSlash(b){
  // 다중 검기 십자포화: 8방향 검기 동시 경고 후 발사
  if(sfx.enemyCore) sfx.enemyCore();
  banner('⚔ 십자 검기','전방향 검기 — 빈틈으로!',850);
  const base=b.angle||0;
...
function set3KekeBombard(b){
  ...
  if(sfx.enemyWarn) sfx.enemyWarn();
  banner('📺 송출좌표 폭격','좌표가 찍힌다 — 비워라',900);
  const cells=[]; for(let cx=0;cx<4;cx++) for(let cy=0;cy<3;cy++) cells.push([W*(0.18+cx*0.21),H*(0.28+cy*0.22)]);
  for(let i=cells.length-1;i>0;i--){ const j=irand(0,i),t=cells[i];cells[i]=cells[j];cells[j]=t; }
  cells.slice(0,7).forEach((c,i)=> setTimeout(()=>{ if(!a3Alive(b))return; warnAoE(c[0],c[1],84,0.7,0.5,22,'송출 폭격','#ff4dd2'); }, i*150));
}
function set3KekeInterference(b){
  // 신호 간섭: 회전 나선 4갈래 (연속 사출)
  if(sfx.enemyCore) sfx.enemyCore();
  banner('📶 신호 간섭','나선 탄막',800);
  for(let t=0;t<14;t++) setTimeout(()=>{ if(!a3Alive(b))return; const base=(b._sigSpin=(b._sigSpin||0)+0.36); for(let k=0;k<4;k++){ const a=base+k/4*TAU; eBullets.push({x:b.x,y:b.y,vx:Math.cos(a)*200,vy:Math.sin(a)*200,r:7,dmg:14,life:3.6,srcName:'신호 간섭',col:'#38e8ff'}); } }, t*90);
}
function set3KekeChannelWave(b){
  // 채널 점거: 한 변에서 빈틈 있는 탄벽이 밀려옴
  if(sfx.enemyCast) sfx.enemyCast();
  banner('📡 채널 점거','탄벽 — 빈틈을 찾아라',950);
  const side=irand(0,3), gap=irand(2,8), dmg=15;
  for(let i=0;i<12;i++){ if(i>=gap&&i<gap+2) continue; let x,y,vx,vy;
    if(side===0){ x=W*(i/11); y=30; vx=0; vy=185; }
    else if(side===1){ x=W*(i/11); y=H-30; vx=0; vy=-185; }
    else if(side===2){ x=30; y=120+(H-180)*(i/11); vx=185; vy=0; }
    else { x=W-30; y=120+(H-180)*(i/11); vx=-185; vy=0; }
    eBullets.push({x,y,vx,vy,r:8,dmg,life:4.4,srcName:'채널 점거',col:'#ff4dd2'}); }
}
function set3KekeOrbBloom(b){
  // 에너지구 만개: 느린 에너지구 4개가 일정 후 전방위 개화
  if(sfx.enemyCore) sfx.enemyCore();
  banner('💠 에너지구 만개','구체가 터진다',900);
  for(let i=0;i<4;i++){ const a=b.angle+i/4*TAU, ox=b.x+Math.cos(a)*30, oy=b.y+Math.sin(a)*30;
    const bo={x:ox,y:oy,vx:Math.cos(a)*90,vy:Math.sin(a)*90,r:10,dmg:13,life:1.6,srcName:'에너지구',col:'#38e8ff'};
    eBullets.push(bo);
    setTimeout(()=>{ if(state!=='play')return; const k=12; for(let j=0;j<k;j++){ const aa=j/k*TAU; eBullets.push({x:bo.x,y:bo.y,vx:Math.cos(aa)*195,vy:Math.sin(aa)*195,r:7,dmg:13,life:3,srcName:'에너지구 파편',col:'#ff4dd2'}); } burst(bo.x,bo.y,'#38e8ff',12,150); }, 1300); }
}

// ── 잡몹 신선화 헬퍼 (보스급 아님, 가벼운 1방씩) ──
function magnetReverseBurst(e){
...
function lagAfterimage(x,y,e){
  ...
  setTimeout(()=>{ if(!enemies.includes(e))return; for(let i=0;i<4;i++){ const a=i/4*TAU+0.4; eBullets.push({x,y,vx:Math.cos(a)*200,vy:Math.sin(a)*200,r:7,dmg:13,life:3,srcName:(e.name||e.label)+' 잔상',col:'#38e8ff'}); } burst(x,y,'#38e8ff',8,120); }, 360);
}
function reflectorCrossBeam(e){
  // 오픈더 반사빔: 반사 자세 중 십자 미니빔
  const base=Math.atan2(player.y-e.y,player.x-e.x);
  for(let k=0;k<2;k++) kijoLaserWarns.push({x:e.x,y:e.y,ang:base+k*Math.PI/2,width:12,range:520,t:0,warn:0.5,color:'#8be8ff',fired:false,sniper:true,dmg:12,srcName:(e.name||e.label)+' 반사빔'});
  if(typeof beep==='function')beep(700,0.05,'sine',0.03);
}
function summonerSandWhirl(e){
  // 알빠노 모래 회오리: 회전 오프셋 2겹 전방위 (느린 탄)
  banner('🌪 모래 회오리','휘몰아치는 모래',700);
  for(let r=0;r<2;r++){ const k=12, off=r*0.26; for(let i=0;i<k;i++){ const a=off+i/k*TAU; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*(150+r*40),vy:Math.sin(a)*(150+r*40),r:7,dmg:11,life:3.8,srcName:e.name||e.label,col:'#e0b85a'}); } }
  burst(e.x,e.y,'#e0b85a',12,150); if(typeof beep==='function')beep(190,0.1,'triangle',0.04);
}

// ===== ACT3 시그니처 메커닉 (티어별 고유 verb) =====
// [엘리트 노잭] 영역 잠식 — 송출 구역이 화면을 먹어들어감 (누적 → 공간 압박)
function act3TruckClaim(e){
  if(countEnemyHazards('poison','송출 점거')>=3) return;   // 송출 장판 최대 3개 유지 (누적 압박 상한)
  if(sfx.enemyCore) sfx.enemyCore();
  const spots=[[W*0.26,H*0.34],[W*0.74,H*0.34],[W*0.26,H*0.72],[W*0.74,H*0.72],[W*0.5,H*0.54]];
...
function onsterTether(e){
  ...
  a3tether={x:clamp(player.x,_mx,W-_mx),y:clamp(player.y,_my,H-_my),r:_tr,t:0,life:e.awakened?4.0:3.4,col:e.awakened?'#ff4dd2':'#8d72ff',owner:e,_chip:0};
  banner('⛓ 사슬 테더','반경 밖으로 못 나간다 — 회피로 끊어라',1100);
  screenShake=Math.max(screenShake||0,8);
}
// [현진 P1] 강제 근접 — 거리두기 봉쇄, 계속 끌려간다 (그랩 락 상태)
function set3GrabLock(b){
  b._grabLockT=3.4; b._grabPullT=0;
  banner('🤜 현진 강제 근접','거리를 둘 수 없다 — 붙어서 버텨라',1100);
  if(sfx.enemyDash) sfx.enemyDash(); screenShake=Math.max(screenShake||0,10);
}
// [번검 P2] 분신 식별 — 동일 외형 분신, 진짜만 데미지(가짜 때리면 반격), 분신도 사격
function set3Identify(b){
  a3Decoys(b);
...
function set3SignalReflect(b){
  ...
  if(sfx.enemyCore) sfx.enemyCore(); burst(b.x,b.y,'#ff4dd2',18,220); screenShake=Math.max(screenShake||0,8);
  const k=14; for(let i=0;i<k;i++){ const a=i/k*TAU; eBullets.push({x:b.x,y:b.y,vx:Math.cos(a)*150,vy:Math.sin(a)*150,r:7,dmg:13,life:3.4,srcName:'신호 역송출',col:'#ff4dd2'}); }
}

// ===================================================================
// ===== 3막 v3 신규 패턴 (난이도 상향: 쿨감 아님 · 신규 패턴 추가) =====
//  · 모든 신규 패턴은 텔레그래프(경고) 필수 · 즉발 억까 금지
//  · 기존 헬퍼(warnAoE/a3Objective/kijoLaserWarns/eBullets/hazards/intentShockwave) 재사용
// ===================================================================

// 플레이어 현재 이동 방향(정규화) — 폭격 예측용. 정지 시 {0,0}
function playerMoveDir(){
  ...
  const m=Math.hypot(mx,my); if(m<0.01) return {x:0,y:0};
...
function act3TruckNewsWall(e){
  ...
  setTimeout(()=>{ if(!a3Alive(e))return;
    for(let i=0;i<n;i++){ if(i>=gap&&i<gap+gapW) continue; let x,y,vx,vy;
      ...
      else if(side===2){ x=20;  y=110+(H-160)*(i/(n-1)); vx=sp; vy=0; }  // 왼→오
      else {              x=W-20; y=110+(H-160)*(i/(n-1)); vx=-sp; vy=0; } // 오→왼
      eBullets.push({x,y,vx,vy,r:8,dmg,life:5.2,srcName:(e.name||e.label||'긴급속보'),col:'#ffd34d'});
    }
...
function onsterChainAnchor(e){
  ...
  for(let i=0;i<n;i++){
    const a=i/n*TAU+rand(-0.3,0.3), rr=rand(150,235);
    a3Objective(clamp(player.x+Math.cos(a)*rr,60,W-60), clamp(player.y+Math.sin(a)*rr,120,H-90),
      {hp:e.awakened?150:128, fuse:e.awakened?6.5:7.2, fail:'aoe', failDmg:e.awakened?32:26,
       label:'사슬 닻', color:'#8d72ff', r:24, owner:e, shootCd});
  }
  banner('⛓ 사슬 닻 '+n+'개','제한시간 내 부숴라 — 못 부수면 사슬 폭발',1050);
  if(typeof beep==='function')beep(110,0.16,'sawtooth',0.05);
}

// ── [중보 온스터] 사슬 미로 — 불규칙 가로/세로 사슬빔, 안전 통로 1줄 ──
function onsterChainMaze(e){
  if(sfx.enemyChain) sfx.enemyChain();
  const col=e.awakened?'#ff4dd2':'#8d72ff', dmg=e.awakened?18:15, warn=e.awakened?0.72:0.84;
  const lane=irand(0,2), safeX=[W*0.2,W*0.5,W*0.8][lane], safeW=130;  // 중앙/좌/우 중 안전통로
  const cast=(extra)=>{
    let placed=0;
    for(let i=0;i<8&&placed<5;i++){ const x=rand(60,W-60); if(Math.abs(x-safeX)<safeW) continue;  // 세로빔(안전통로 비움)
      kijoLaserWarns.push({x,y:0,ang:Math.PI/2,width:16,range:H+40,t:0,warn:warn+(extra||0),color:col,fired:false,sniper:true,dmg,srcName:'사슬 미로'}); placed++; }
    const hn=1+irand(0,1);                                            // 가로빔 1~2줄(회피 판단)
    for(let i=0;i<hn;i++){ const y=rand(H*0.30,H*0.78);
      kijoLaserWarns.push({x:0,y,ang:0,width:16,range:W+40,t:0,warn:warn+(extra||0),color:col,fired:false,sniper:true,dmg,srcName:'사슬 미로'}); }
  };
  cast(0);
  banner('⛓ 사슬 미로','안전 통로로 빠져나가라',1000);
  if(e.awakened) setTimeout(()=>{ if(!a3Alive(e))return; cast(0.05); banner('⛓ 2차 사슬','반대편에서 또 온다',850); }, 800);
  if(typeof beep==='function')beep(95,0.14,'sawtooth',0.05);
}
...
function onsterBrokenChains(e){
  ...
  banner('⛓ 끊어진 사슬','파편이 날아온다',900);
  const ox=e.x, oy=e.y;
  setTimeout(()=>{ if(!a3Alive(e))return;
    const base=Math.atan2(player.y-oy,player.x-ox);
    for(let s=-1;s<=1;s++){ const a=base+s*0.5;
      for(let i=0;i<3;i++) eBullets.push({x:ox,y:oy,vx:Math.cos(a)*(150+i*14),vy:Math.sin(a)*(150+i*14),r:8,dmg:18,life:4.2,home:0.4,srcName:'끊어진 사슬',col:'#ff4dd2'});
    }
    burst(ox,oy,'#ff4dd2',12,170); if(typeof beep==='function')beep(120,0.12,'sawtooth',0.05);
  }, 600);   // 0.6초 경고 후 발사 (즉발 금지)
}
...
function onsterChainCage(e){
  ...
  a3tether={x:clamp(player.x,_mx,W-_mx),y:clamp(player.y,_my,H-_my),r:r0,r0:r0,rMin:rMin,t:0,life:e.awakened?5.0:4.4,col:'#ff4dd2',owner:e,_chip:0,shrink:true,ring:true,_ringT:0,_ringAng:0};
  banner('⛓ 사슬 수축 감옥','원이 좁혀진다 — 한계 전에 회피로 끊어라',1200);
  screenShake=Math.max(screenShake||0,10);
  if(typeof beep==='function')beep(90,0.2,'sawtooth',0.06);
}
...
function onsterBindExecute(e){
  ...
  banner('⛓ 사슬 속박','잠긴다 — 풀리면 즉시 자리를 떠라',1000);
  if(typeof beep==='function')beep(110,0.16,'sawtooth',0.05);
  const lx=player.x, ly=player.y, dmg=e.awakened?30:25;
  setTimeout(()=>{ if(!a3Alive(e))return;
    hazards.push({x:clamp(lx,34,W-34),y:clamp(ly,90,H-50),t:0,warnT:0.7,liveT:0.5,r:130,hit:false,dmg,srcName:'사슬 처형',col:'#ff4dd2'});
    screenShake=Math.max(screenShake||0,9); if(typeof beep==='function')beep(60,0.3,'sawtooth',0.06);
  }, 700);   // 잠금 해제(0.6s) 후 슬램 경고 — 풀린 뒤 빠져나갈 창 확보
}
...
function onsterCounterWhip(e){
  ...
  hazards.push({kind:'spinbar',owner:e,x:e.x,y:e.y,len:e.awakened?200:165,wid:22,ang:rand(0,TAU),rot:-(e.awakened?2.8:2.4),t:0,warnT:0.7,liveT:life,dmg,hitCd:0,srcName:'역회전 사슬',col:e.color});
  banner('⛓ 역회전 사슬','빈틈을 쫓아 돌아라',850);
}
...
function onsterReelToMinions(e){
  ...
  let tx=e.x, ty=e.y;
  if(mins.length){ tx=mins.reduce((s,m)=>s+m.x,0)/mins.length; ty=mins.reduce((s,m)=>s+m.y,0)/mins.length; }
  banner('🪝 사슬 견인','무리 쪽으로 끌려간다 — 회피로 끊어라',1000);
  if(typeof beep==='function')beep(90,0.2,'sawtooth',0.06);
  if((player.dodging||0)<=0){
    const a=Math.atan2(ty-player.y,tx-player.x);
    player.x=clamp(player.x+Math.cos(a)*200,player.r,W-player.r);
    player.y=clamp(player.y+Math.sin(a)*200,player.r,H-player.r);
    burst(player.x,player.y,'#8d72ff',16,210);
    screenShake=Math.max(screenShake||0,6);
  }
}

// ── [현진 P1] 벽치기 돌진 — 0.8초 경고 → 돌진 → 벽 방향 넉백(벽 근처 추가 충격파) ──
function set3HyeonjinWallSlam(b){
  if(sfx.enemyDash) sfx.enemyDash();
  const ang=Math.atan2(player.y-b.y,player.x-b.x), dx=Math.cos(ang), dy=Math.sin(ang);
  const ox=b.x, oy=b.y, len=Math.max(W,H);
  // 돌진 경로 텔레그래프 + 직격 판정 (band 재사용, 직격 20)
  hazards.push({kind:'band',x:ox+dx*len*0.5,y:oy+dy*len*0.5,ang,len,wid:70,t:0,warnT:0.8,liveT:0.32,dmg:20,hit:false,srcName:'현진 벽치기',col:b.color||'#ff4dd2'});
  banner('🏃 현진 돌진','경로를 비워라 — 벽 끼임 주의',1000);
  if(typeof beep==='function')beep(70,0.18,'sawtooth',0.05);
  setTimeout(()=>{ if(!a3Alive(b))return;
    b.x=clamp(ox+dx*280,b.r,W-b.r); b.y=clamp(oy+dy*200,b.r,H*0.6);   // 돌진 연출
    burst(b.x,b.y,b.color||'#ff4dd2',18,240); screenShake=Math.max(screenShake||0,12);
    const px=player.x-ox, py=player.y-oy, along=px*dx+py*dy, perp=Math.abs(-px*dy+py*dx);
    if(along>-40 && along<len && perp<48 && (player.dodging||0)<=0){   // 경로상 피격 시 넉백
      player.x=clamp(player.x+dx*200,player.r,W-player.r);
      player.y=clamp(player.y+dy*200,player.r,H-player.r);
      burst(player.x,player.y,'#ff4dd2',12,180);
      if(player.x<72||player.x>W-72||player.y<132||player.y>H-72) intentShockwave(player.x,player.y,112,16,'벽 충격파');  // 벽 충격파 16
    }
  }, 800);
}

// ── [번검 P2] 삼연참 — 1·2·3 순서로 검기 '선' 예고 후 같은 순서로 폭발 ──
function set3BeongeomTripleSlash(b){
  if(sfx.enemyLaser) sfx.enemyLaser();
  banner('⚔ 삼연참','1·2·3 순서로 베어온다',1000);
  const base=Math.atan2(player.y-b.y,player.x-b.x), angs=[base-0.5, base, base+0.5];
  angs.forEach((ang,i)=> setTimeout(()=>{ if(!a3Alive(b))return;
    kijoLaserWarns.push({x:b.x,y:b.y,ang,width:20,range:840,t:0,warn:0.62,color:'#38e8ff',fired:false,sniper:true,dmg:18,srcName:'삼연참 '+(i+1)});
    if(typeof beep==='function')beep(420+i*120,0.06,'square',0.04);
  }, i*360));   // 순차 예고 → 순차 폭발
}
...
// ── [세트3 P1 현진] 형제 협공 — 양옆에서 두 형제가 난입해 3방향 동시 수렴 검기 ──
function set3BrotherCross(b){
  if(sfx.enemyCore) sfx.enemyCore();
  banner('👥 형제 협공','세 방향에서 동시에 — 수렴점을 벗어나라',1100);
  const srcs=[
    {x:b.x,y:b.y,col:'#38e8ff'},
    {x:clamp(player.x-380,40,W-40),y:clamp(player.y-130,80,H*0.6),col:'#ff4dd2'},
    {x:clamp(player.x+380,40,W-40),y:clamp(player.y-130,80,H*0.6),col:'#ffd34d'}
  ];
  srcs.forEach((s)=>{
    burst(s.x,s.y,s.col,14,180);
    const pa=Math.atan2(player.y-s.y,player.x-s.x);
    kijoLaserWarns.push({x:s.x,y:s.y,ang:pa,width:20,range:980,t:0,warn:0.9,color:s.col,fired:false,sniper:true,dmg:20,srcName:'형제 협공'});
  });
  if(typeof beep==='function')beep(180,0.12,'square',0.05);
}
// ── [세트3 P2 번검] 거합 · 잔상 — 빠르게 대시하며 지나간 자리가 한 박자 늦게 갈라짐 ──
function set3IaidoAfterimage(b){
  if(sfx.enemyLaser) sfx.enemyLaser();
  banner('⚔ 거합 · 잔상','지나간 자리가 늦게 터진다 — 있던 곳을 피해라',1100);
  const startA=Math.atan2(player.y-b.y,player.x-b.x);
  for(let i=0;i<4;i++){
    setTimeout(()=>{ if(!a3Alive(b))return;
      const ang=startA+rand(-0.7,0.7);
      const ox=clamp(b.x+rand(-140,140),60,W-60), oy=clamp(b.y+rand(-70,90),90,H*0.55);
      b.x=ox; b.y=oy; burst(ox,oy,'#38e8ff',12,160);
      hazards.push({kind:'band',x:ox,y:oy,ang,len:Math.max(W,H),wid:30,t:0,warnT:0.55,liveT:0.3,dmg:18,hit:false,srcName:'거합 잔상',col:'#38e8ff'});
      if(typeof beep==='function')beep(620,0.05,'square',0.04);
    }, i*420);
  }
}
...
function set3LivePoll(b){
  ...
  const lx=W*0.28, rx=W*0.72, cy=H*0.5, R=215;
  set3Poll={startMs:performance.now(), dur:1500, lx,rx,cy,R,dangerLeft, owner:b};
  banner('🗳 생방송 투표','채팅이 한 쪽을 고른다 — 채워지는 쪽을 피해라',1300);
  if(typeof beep==='function')beep(520,0.08,'triangle',0.05);
  const dxp=dangerLeft?lx:rx;
  setTimeout(()=>{ if(!a3Alive(b)){ set3Poll=null; return; }
    warnAoE(dxp,cy,R,0.55,0.6,28,'생방송 투표','#ff4dd2');
    if(typeof sfx!=='undefined'&&sfx.enemyExplode) sfx.enemyExplode();
    if(typeof beep==='function')beep(150,0.16,'sawtooth',0.06);
    set3Poll=null;
  }, 1500);
}
...
function act3TruckMegaBeam(e){
  ...
  hazards.push({kind:'megabeam',x:e.x,y:e.y,ang,rot:spin,len:Math.max(W,H)*1.45,width:84,
    warnT:1.35,liveT:1.05,t:0,dmg:Math.max(22,Math.round((e.dmg||16)*1.5)),
    srcName:(e.name||e.label||'노잭')+' 대형빔',seed:rand(0,TAU),hitCd:0,col:'#58d8ff'});
  burst(e.x,e.y,'#58d8ff',16,140);
  if(typeof beep==='function')beep(64,0.55,'sawtooth',0.06);
}
...
function onsterCrossLaser(e){
  ...
  onsterCross={owner:e,x:e.x,y:e.y,t:0,warnT:1.2,life:ph===2?5.0:4.2,
    a1:rand(0,TAU), a2:rand(0,TAU)+Math.PI/2,
    rot1:dir*0.5, rot2:dir*0.64,          // 같은 방향·미세 속도차 → 빈틈이 닫히지 않음
    width:ph===2?24:22, dmg:ph===2?18:14, _hitCd:0, col:'#8d72ff'};
  banner('✖ 사슬 교차','같은 방향으로 도는 두 빔 — 사이 빈 칸으로',1300);
  if(typeof beep==='function')beep(150,0.2,'square',0.05);
}
...
function onsterDeepBreath(e){
  ...
  onsterBreath={owner:e,t:0,inhaleT:1.6,exhaleT:1.35,pull:ph===2?120:95,
    dmg:ph===2?10:8,dir:Math.atan2(player.y-e.y,player.x-e.x),_fired:false,ph};
  banner('🌀 깊은 들숨','빨려든다 — 옆으로 빠져 숨결을 피해라',1100);
  if(typeof beep==='function')beep(78,0.55,'sine',0.05);
}
function onsterBreathBlast(e,dir,dmg,ph){
...
// ── [번검 P2 · 필드] 반반 시간 — 좌/우 절반 탄속이 다른 '필드'는 번검 페이즈 내내 상시(updateA3Systems에서 생성). ──
// ── 이 스킬은 느림/빠름 '두 구역의 위치를 맞바꾸는' 역할 + 읽을거리용 가벼운 대칭 링을 깐다 ──
function set3HalfSwap(b){
  if(!set3Half) set3Half={field:true,splitX:W/2,leftSlow:Math.random()<0.5,owner:b,t:0};
  set3Half.leftSlow=!set3Half.leftSlow;
  if(sfx.enemyGlitch) sfx.enemyGlitch();
  banner('⏳ 시간 역전','느림과 빠름이 자리를 바꾼다 — 다시 읽어라',1300);
  screenShake=Math.max(screenShake||0,6);
  // 필드 위에서 좌우 속도차가 드러나도록 가벼운 대칭 링
  const cx=b.x, cy=b.y, k=14;
  for(let i=0;i<k;i++){ const a=i/k*TAU;
    eBullets.push({x:cx,y:cy,vx:Math.cos(a)*150,vy:Math.sin(a)*150,r:8,dmg:14,life:5.4,srcName:'번검 시간 베기',col:'#38e8ff',style:'shard'}); }
...
function set3Genesis(b){
  if(sfx.enemyCast) sfx.enemyCast();
  banner('✦ 제네시스','빛기둥이 내리친다 — 빈 칸으로',1100);
  const cols=7, gap=W/cols;
  let safe1=Math.floor(rand(0,cols)), safe2=(safe1+2+Math.floor(rand(0,cols-3)))%cols;
  if(safe2===safe1) safe2=(safe1+3)%cols;
  for(let c=0;c<cols;c++){
    if(c===safe1||c===safe2) continue;
    hazards.push({kind:'genpillar',x:gap*c+gap/2,w:gap*0.80,
      warnT:0.92+(c%2)*0.13,liveT:0.55,t:0,dmg:20,srcName:'제네시스',seed:rand(0,TAU)});
  }
...
function set3BlackMage(b){
  ...
  if(typeof beep==='function'){ beep(180,0.2,'sine',0.05); setTimeout(()=>beep(120,0.3,'sawtooth',0.05),760); }
}

function updateA3Systems(dt){
  if(state!=='play'){ a3veil=null; a3seq=null; a3strike=null; a3tether=null; set3Poll=null; set3Ring=null; set3Ad=null; onsterCross=null; onsterBreath=null; set3Half=null; set3Reaper=null; if(player) player._adBlockT=0; return; }
  if(player&&player._adBlockT>0) player._adBlockT=Math.max(0,player._adBlockT-dt);

  // ── 온스터 사슬 교차빔 ──
  if(onsterCross){
    const C=onsterCross; C.t+=dt; C._hitCd-=dt;
    if(!C.owner||!a3Alive(C.owner)){ onsterCross=null; }
    else {
      C.x=C.owner.x; C.y=C.owner.y;
      const active=C.t>=C.warnT;
      if(active){ C.a1+=C.rot1*dt; C.a2+=C.rot2*dt; }
      if(active && C._hitCd<=0){
        const px=player.x-C.x, py=player.y-C.y;
        const under=(ang)=>Math.abs(-px*Math.sin(ang)+py*Math.cos(ang))<C.width;
        if((under(C.a1)||under(C.a2)) && (player.dodging||0)<=0){ hurtPlayer(C.dmg,'사슬 교차'); C._hitCd=0.42; }
      }
      if(C.t>=C.warnT+C.life){ burst(C.x,C.y,C.col,14,180); onsterCross=null; }
    }
  }
  // ── 온스터 깊은 숨결 ──
  if(onsterBreath){
    const B=onsterBreath; B.t+=dt;
    if(!B.owner||!a3Alive(B.owner)){ onsterBreath=null; }
    else {
      const ox=B.owner.x, oy=B.owner.y;
      if(B.t<B.inhaleT){
        if((player.dodging||0)<=0){
          const dx=ox-player.x, dy=oy-player.y, d=Math.hypot(dx,dy)||1;
          // 일정 거리 안쪽(점착)으로는 끌지 않음 → point-blank 흡입 방지
          if(d>150){ const f=B.pull*(0.45+0.55*(B.t/B.inhaleT)); player.x=clamp(player.x+dx/d*f*dt,player.r,W-player.r); player.y=clamp(player.y+dy/d*f*dt,player.r,H-player.r); }
        }
        if(Math.random()<0.7){ const a=rand(0,TAU), rr=rand(170,360); particles.push({x:ox+Math.cos(a)*rr,y:oy+Math.sin(a)*rr,vx:-Math.cos(a)*240,vy:-Math.sin(a)*240,life:0.55,max:0.55,color:'#b7a4ff',r:3}); trimArrayHead(particles,PERF_LIMITS.particles); }
      } else if(!B._fired){
        B._fired=true;   // 방향은 들숨 시작 때 고정값(B.dir) 사용 — 옆으로 빠지면 회피됨
        onsterBreathBlast(B.owner,B.dir,B.dmg,B.ph);
      }
      if(B.t>=B.inhaleT+B.exhaleT) onsterBreath=null;
    }
  }
  // ── 번검(P2) 반반 시간 필드 — 페이즈 동안 상시 ──
  if(boss && boss.key==='set3' && (boss.setPhase||1)===2){
  ...
  if(set3Reaper){
    ...
    if(!S.owner||!a3Alive(S.owner)){ if(player) player._adBlockT=0; set3Reaper=null; }
      ...
      S.mx+=(player.x-S.mx)*Math.min(1,dt*6); S.my+=(player.y-S.my)*Math.min(1,dt*6);
      if(!S._bound){ S._bound=true; player._adBlockT=Math.max(player._adBlockT||0,S.lockT+S.freezeT+0.1); }
      if(S.t>=S.lockT){ S.phase='freeze'; S.t=0; if(sfx.enemyWarn) sfx.enemyWarn(); banner('🔒 미래 확정','그 자리에서 벗어나라!',900); if(typeof beep==='function')beep(90,0.25,'square',0.06); }
    } else if(S.phase==='freeze'){
      if(S.t>=S.freezeT){ S.phase='blast'; S.t=0;
        if(dist2(player.x,player.y,S.mx,S.my)<S.r*S.r) hurtPlayer(S.dmg,'검은 마법 · 미래시');
        burst(S.mx,S.my,'#9146ff',28,260); screenShake=Math.max(screenShake||0,10);
        if(sfx.enemyExplode) sfx.enemyExplode(); if(typeof beep==='function')beep(70,0.4,'sawtooth',0.06);
      }
    } else { if(S.t>=S.blastT) set3Reaper=null; }
  }
  ...
  if(a3tether){
    a3tether.t+=dt;
    if(a3tether.shrink){ const k=clamp(a3tether.t/Math.max(0.1,a3tether.life),0,1); a3tether.r=lerp(a3tether.r0,a3tether.rMin,k); }
    const tx=a3tether.x, ty=a3tether.y, r=a3tether.r;
    if(a3tether.ring){
      a3tether._ringAng=(a3tether._ringAng||0)+dt*1.6;
      a3tether._ringT=(a3tether._ringT||0)-dt;
      if(a3tether._ringT<=0){ a3tether._ringT=0.9; const k=8; for(let i=0;i<k;i++){ const a=a3tether._ringAng+i/k*TAU; eBullets.push({x:tx+Math.cos(a)*r,y:ty+Math.sin(a)*r,vx:Math.cos(a+Math.PI/2)*120,vy:Math.sin(a+Math.PI/2)*120,r:7,dmg:13,life:2.6,srcName:'수축 감옥 사슬',col:a3tether.col}); } if(typeof beep==='function')beep(160,0.05,'square',0.03); }
    }
    const dx=player.x-tx, dy=player.y-ty, d=Math.hypot(dx,dy);
    if((player.dodging||0)>0 && d>r*0.80){ banner('⛓ 테더 절단!','사슬을 끊었다',800); burst(tx,ty,a3tether.col,18,220); if(sfx.enemyChain) sfx.enemyChain(); a3tether=null; }
    else {
      if(d>r){ const a=Math.atan2(dy,dx); player.x=clamp(tx+Math.cos(a)*r,player.r,W-player.r); player.y=clamp(ty+Math.sin(a)*r,player.r,H-player.r); a3tether._chip-=dt; if(a3tether._chip<=0){ hurtPlayer(8,'사슬 테더'); a3tether._chip=0.5; } }
      if(a3tether.t>=a3tether.life){ burst(tx,ty,a3tether.col,12,160); a3tether=null; }
    }
  }
}
// 검은 마법 픽셀 룬(해골)
function drawReaperSigil(cx,cy,col,glow){
  const pal={'1':col,'2':_shade(col,-0.45),'3':'#0a0612'};
  const rows=[".111.","13331","13331","11111",".131.","1.1.1"];
  ctx.save(); ctx.globalAlpha=0.85+0.15*(glow||0); ctx.translate(cx,cy);
  pxDraw(rows,6,pal);
  ctx.restore();
}
...
function drawA3World(){
  ...
  if(a3tether){
    const col=a3tether.col||'#8d72ff';
    ctx.save();
    ctx.globalAlpha=0.85; ctx.strokeStyle=col; ctx.lineWidth=3; ctx.setLineDash([8,6]); ctx.beginPath(); ctx.moveTo(a3tether.x,a3tether.y); ctx.lineTo(player.x,player.y); ctx.stroke(); ctx.setLineDash([]);
    ctx.globalAlpha=0.5+0.22*Math.sin(performance.now()/120); ctx.lineWidth=4; ctx.beginPath(); ctx.arc(a3tether.x,a3tether.y,a3tether.r,0,TAU); ctx.stroke();
    ctx.globalAlpha=0.10; ctx.fillStyle=col; ctx.beginPath(); ctx.arc(a3tether.x,a3tether.y,a3tether.r,0,TAU); ctx.fill();
    ctx.restore();
  }
  if(boss&&boss.key==='set3'&&(boss.reflectT||0)>0){
    ctx.save(); ctx.globalAlpha=0.5+0.3*Math.sin(performance.now()/80); ctx.strokeStyle='#ff4dd2'; ctx.lineWidth=4; ctx.beginPath(); ctx.arc(boss.x,boss.y,boss.r+12,0,TAU); ctx.stroke(); ctx.restore();
  }
  // ── 온스터 사슬 교차빔 ──
  if(onsterCross){
    const C=onsterCross, warn=C.t<C.warnT, col=C.col;
    const drawArm=(ang)=>{
      const len=Math.max(W,H)*1.2, w=C.width, grid=Math.max(6,w*0.52);
      ctx.save(); ctx.translate(C.x,C.y); ctx.rotate(ang);
      ctx.globalAlpha=warn?(0.13+0.12*Math.abs(Math.sin(C.t*16))):0.42;
      ctx.fillStyle='rgba(141,114,255,0.22)'; ctx.fillRect(-len,-w*0.8,len*2,w*1.6);
      const a=warn?0.42:0.95;
      for(let x=-len;x<=len;x+=grid){ const cell=Math.round(x/grid);
        ctx.globalAlpha=a; ctx.fillStyle=(cell%2===0)?col:_shade(col,-0.42);
        ctx.fillRect(Math.round(x-grid/2),Math.round(-w*0.72),Math.ceil(grid*0.8),Math.ceil(w*1.44));
        if(!warn){ ctx.globalAlpha=a; ctx.fillStyle=_shade(col,0.6); ctx.fillRect(Math.round(x-grid*0.16),-3,Math.max(2,Math.round(grid*0.3)),6); }
      }
      ctx.restore();
    };
    drawArm(C.a1); drawArm(C.a2);
    ctx.save(); ctx.globalAlpha=0.92; pxBlob(C.x,C.y,16,16,5,col,_shade(col,-0.4)); ctx.restore();
  }
  // ── 온스터 깊은 숨결 ──
  if(onsterBreath){
    const B=onsterBreath, ox=B.owner?B.owner.x:W/2, oy=B.owner?B.owner.y:H/2;
    if(B.t<B.inhaleT){
      const k=clamp(B.t/B.inhaleT,0,1);
      ctx.save();
      for(let r=0;r<4;r++){ const rr=Math.max(20,340*(1-k*0.5)-r*70), n=10;
        for(let i=0;i<n;i++){ const a=i/n*TAU+B.t*(1.6+r*0.4)+r, px=ox+Math.cos(a)*rr, py=oy+Math.sin(a)*rr;
          ctx.globalAlpha=0.32+0.42*k; ctx.fillStyle=r%2?'#a98bff':'#7a5fd0';
          ctx.fillRect(Math.round(px-3),Math.round(py-3),6,6); } }
      ctx.globalAlpha=0.5+0.4*Math.abs(Math.sin(B.t*10)); ctx.strokeStyle='#b7a4ff'; ctx.lineWidth=2;
      ctx.beginPath(); ctx.arc(ox,oy,28+10*Math.sin(B.t*8),0,TAU); ctx.stroke();
      ctx.restore();
    } else {
      const k=clamp((B.t-B.inhaleT)/0.25,0,1);
      ctx.save(); ctx.globalAlpha=(1-k)*0.6; ctx.translate(ox,oy); ctx.rotate(B.dir||0); ctx.fillStyle='#cbb8ff';
      for(let g=0;g<6;g++){ const gx=14+g*16; ctx.fillRect(gx,-Math.round(6+g*4),10,Math.round(12+g*8)); }
      ctx.restore();
    }
  }
  // ── 번검 반반 시간 ──
  if(set3Half){
    const S=set3Half, x=S.splitX, slowLeft=S.leftSlow;
    ctx.save();
    ctx.globalAlpha=0.05;
    ctx.fillStyle=slowLeft?'#2f6aa8':'#a8327a'; ctx.fillRect(0,0,x,H);
    ctx.fillStyle=slowLeft?'#a8327a':'#2f6aa8'; ctx.fillRect(x,0,W-x,H);
    const grid=12;
    for(let y=0;y<H;y+=grid){ const phase=Math.floor(y/grid)%2;
      ctx.globalAlpha=0.85; ctx.fillStyle=phase?'#cfe6ff':'#9146ff';
      ctx.fillRect(Math.round(x-grid/2),y,grid,Math.ceil(grid*0.85)); }
    ctx.globalAlpha=0.92; ctx.font='bold 19px sans-serif'; ctx.textAlign='center';
    ctx.fillStyle='#9fd6ff'; ctx.fillText(slowLeft?'🐌 느림':'⚡ 빠름', x*0.5, 42);
  ...
  if(set3Reaper){
    ...
    const col=S.phase==='blast'?'#c46bff':(S.phase==='freeze'?'#ff3b6b':'#9146ff');
    ctx.save();
    ctx.globalAlpha=0.10+0.18*flash; ctx.fillStyle=col;
    ctx.beginPath(); ctx.arc(S.mx,S.my,S.r,0,TAU); ctx.fill();
    ctx.globalAlpha=0.6+0.4*flash; ctx.strokeStyle=col; ctx.lineWidth=3; ctx.setLineDash([10,7]);
    ctx.beginPath(); ctx.arc(S.mx,S.my,S.r,0,TAU); ctx.stroke(); ctx.setLineDash([]);
    drawReaperSigil(S.mx,S.my, S.phase==='blast'?'#ffffff':col, flash);
    if(S.phase==='blast'){ const k=clamp(S.t/S.blastT,0,1);
      ctx.globalAlpha=(1-k)*0.7; ctx.strokeStyle='#fff'; ctx.lineWidth=5;
      ctx.beginPath(); ctx.arc(S.mx,S.my,S.r*k,0,TAU); ctx.stroke(); }
    ctx.restore();
  }
  if(set3Poll){
    const P=set3Poll, k=clamp((performance.now()-P.startMs)/Math.max(1,P.dur),0,1);
    const sides=[{x:P.lx,danger:P.dangerLeft},{x:P.rx,danger:!P.dangerLeft}];
    ctx.save();
    sides.forEach(sd=>{
      // 두 구역: 위험 쪽은 채워지고(빨강), 안전 쪽은 옅게(청록)
      const col=sd.danger?'#ff4dd2':'#38e8ff';
      ctx.globalAlpha=0.10+(sd.danger?0.22*k:0.04);
      ctx.fillStyle=col; ctx.beginPath(); ctx.arc(sd.x,P.cy,P.R,0,TAU); ctx.fill();
      ctx.globalAlpha=0.85; ctx.strokeStyle=col; ctx.lineWidth=3;
      ctx.beginPath(); ctx.arc(sd.x,P.cy,P.R,0,TAU); ctx.stroke();
      // 위험 쪽 투표 게이지(아래에서 차오름)
      if(sd.danger){
        ctx.globalAlpha=0.9; ctx.fillStyle=col;
        const bw=P.R*1.2, bh=10, bx=sd.x-bw/2, by=P.cy+P.R+14;
        ctx.fillRect(bx,by,bw*k,bh);
        ctx.globalAlpha=0.5; ctx.strokeStyle='#fff'; ctx.lineWidth=2; ctx.strokeRect(bx,by,bw,bh);
      }
      ctx.globalAlpha=0.95; ctx.fillStyle='#fff'; ctx.font='bold 16px sans-serif'; ctx.textAlign='center';
      ctx.fillText(sd.danger?'🗳 '+Math.round(k*100)+'%':'안전?', sd.x, P.cy+6); ctx.textAlign='left';
    });
    ctx.restore();
  }
  if(set3Ring){
...
function updateIntentPatterns(e,dt){
  ...
  if(e.type==='goblin_warrior'&&cd('_slamCd',8)){
  ...
  }else if(e.type==='rhino_beetle'&&(!e.cs||e.cs==='approach'||e.cs==='rest')&&cd('_intentChargeCd',7.5)){
    ...
    setIntent(e,'🍎','낙과',0.95,()=>{
      ...
      const ra=Math.atan2(player.y-e.y,player.x-e.x); e._rollAx=Math.cos(ra); e._rollAy=Math.sin(ra); e._rollT=0.7; burst(e.x,e.y,'#ff4d6d',10,170);   // 낙과 후 굴러 들이받기
      e._fallCd=4.5;
    });
...
let eliteIntro=null, slowmoT=0, bagjeinRewind=null;
...
function startCombat(kind, fresh){
  ...
  roomCleared=false; combatClearGrace=false; roomIsBoss=(kind==='boss'); roomIsMidboss=(kind==='midboss'); kills=0; boss=null; roomHadElite=false; roomEliteKind=null; eliteIntro=null; timeScale=1; slowmoT=0; bagjeinRewind=null;
  bossIntroToken++;
  bossIntroSeen={};
...
function fireCleaver(e,ang,opts){
  ...
  const a=aimPlayerLeadAngle(e,enr?0.18:0.14)+(opts.spread||0);
  const sp=enr?305:270;
...
function hurtPlayer(dmg, src){
  if(state!=='play'||roomCleared||combatClearGrace||roomPreviewT>0) return;
...
function hyechulCreepTide(e,ph){
  // [2페+] 점막 잠식: 한쪽 끝에서 점막이 밀려와 깨끗한 바닥을 좁힌다. 점막 위=둔화. 선단을 쏘면 밀려난다.
  if(hazards.some(h=>h.kind==='creepTide')) return;   // 동시 1개
  const fromLeft=Math.random()<0.5, edge=fromLeft?40:W-40, dir=fromLeft?1:-1;
  const maxFrac=ph>=3?0.86:0.78, maxFront=edge+dir*(W-80)*maxFrac;
  hazards.push({kind:'creepTide',dir,edge,front:edge,maxFront,spd:ph>=3?150:120,
    t:0,warnT:0.85,life:ph>=3?13:11,phase:'advance',seed:rand(0,TAU)});
  banner('🟣 점막 잠식','바닥이 먹힌다 — 선단을 쏴서 밀어내라!',1100);
  if(typeof beep==='function')beep(70,0.4,'sawtooth',0.06); screenShake=Math.max(screenShake||0,5);
}
...
function eliteKindOf(e){
  ...
  if(e&&(e.eliteKind==='act3_truck'||e.type==='act3_truck')) return 'act3_truck';
...
function eliteDisplayName(kind){
  if(kind==='yanggaeng') return '양갱';
  if(kind==='act3_truck') return '노잭';
  return '자잘자';
}
function eliteClearText(kind){
  return eliteDisplayName(kind)+' 격파!';
}
function eliteRewardTitle(kind){
  return '⚔️ '+eliteDisplayName(kind)+' 보상';
}
...
function killEnemy(e){
  ...
  if(e.eliteViewer){
    const ek=eliteKindOf(e);
    banner(eliteDisplayName(ek)+' 처치!','',1100);
    spawnDeathBubble(e.x, e.y-e.r-12, eliteDefeatBubble(ek), 3.4);
  }
  if(!isSummon){
...
function beginKijoGaze(b){
  return setIntent(b,'👁','보지 마',1.1,()=>{
    b.gazeT=b.enraged?3.4:2.8; b.gazeStareT=0; b.gazeFireT=0.6;
    banner('👁 보지 마!','커서를 키죠에서 떼고 눈먼 사격을',1100);
    if(typeof beep==='function')beep(120,0.3,'sawtooth',0.06); screenShake=Math.max(screenShake||0,6);
    b._gazeCd=14;
  });
}
function beginKijoStage(b){
  return setIntent(b,'🎭','무대 암전',1.2,()=>{
    b.stageT=b.enraged?5.6:4.6; b.stageHitT=0;
    const n=b.enraged?2:1, spots=[];
    for(let i=0;i<n;i++) spots.push({x:rand(140,W-140),y:rand(170,H-150),r:120});
    b.stageSpots=spots;
    banner('🎭 무대 암전','조명을 피해 어둠에 숨어라',1200);
    if(typeof beep==='function')beep(70,0.45,'sine',0.06); screenShake=Math.max(screenShake||0,7);
    b._stageCd=18;
  });
}
function updateKijoGaze(b,dt){
  if(!(b.gazeT>0)) return;
  b.gazeT-=dt;
  const aimAng=Math.atan2(mouseY-player.y,mouseX-player.x);
  const toBoss=Math.atan2(b.y-player.y,b.x-player.x);
  const off=Math.abs(angleDiff(aimAng,toBoss));
  b.gazeAimOff=off;
  const cone=0.46, looking=off<cone;
  const grace=b.enraged?0.42:0.55;   // 유예: 콘 안에서 이만큼 버텨야 피해 시작
  if(looking){
    b.gazeCharge=Math.min(1,(b.gazeCharge||0)+dt/grace);
    if(b.gazeCharge>=1){
      b.gazeStareT=(b.gazeStareT||0)+dt;
      const tick=b.enraged?0.78:0.95;
      if(b.gazeStareT>=tick){ b.gazeStareT-=tick; hurtPlayer(b.enraged?16:12,'키죠의 시선'); hitFlash=Math.max(hitFlash||0,0.22); }
    }
  } else {
    b.gazeCharge=Math.max(0,(b.gazeCharge||0)-dt*2.2);   // 콘 벗어나면 빠르게 식음
    b.gazeStareT=Math.max(0,(b.gazeStareT||0)-dt*1.6);
  }
  // 충전 중 상승 경고음
  if(looking && (b.gazeCharge||0)<1){ b._gazeBeepT=(b._gazeBeepT||0)-dt; if(b._gazeBeepT<=0){ b._gazeBeepT=0.11; if(typeof beep==='function')beep(300+(b.gazeCharge||0)*430,0.05,'square',0.03); } }
  // 시선 동안 가벼운 견제 링(눈먼 사격 + 회피 강제)
  b.gazeFireT=(b.gazeFireT==null?0.7:b.gazeFireT)-dt;
  if(b.gazeFireT<=0){ b.gazeFireT=b.enraged?0.78:1.0; kijoRing(b.x,b.y,b.enraged?10:8,148,12,(b.angle||0)*1.3); }
  if(b.gazeT<=0){ b.gazeStareT=0; b.gazeCharge=0; }
}
function updateKijoStage(b,dt){
  if(!(b.stageT>0)) return;
  b.stageT-=dt;
  const spots=b.stageSpots||[]; let inLight=false;
  const spd=b.enraged?108:88;
  for(const s of spots){
    const dx=player.x-s.x, dy=player.y-s.y, d=Math.hypot(dx,dy)||1;
    s.x=clamp(s.x+dx/d*spd*dt,60,W-60); s.y=clamp(s.y+dy/d*spd*dt,120,H-80);
    if(dist2(player.x,player.y,s.x,s.y)<s.r*s.r) inLight=true;
  }
  if(inLight){ b.stageHitT=(b.stageHitT||0)-dt; if(b.stageHitT<=0){ b.stageHitT=0.45; hurtPlayer(b.enraged?15:11,'스포트라이트'); hitFlash=Math.max(hitFlash||0,0.22); } }
  if(b.stageT<=0) b.stageSpots=null;
}
function updateBossIntentPatterns(b,dt){
  ...
  if(b.key==='kijo'){
    ...
    else if((b.gazeT||0)<=0&&(b.stageT||0)<=0&&cd('_gazeCd',14)) beginKijoGaze(b);
...
function gAimed(b,k,dmg){ const pa=Math.atan2(player.y-b.y,player.x-b.x); for(let i=0;i<k;i++) gShot(b.x,b.y,pa+(i-(k-1)/2)*0.18,235,7,dmg||9); }
function gHoming(b,dmg){ const pa=Math.atan2(player.y-b.y,player.x-b.x); for(let i=0;i<5;i++) gShot(b.x,b.y,pa+(i-2)*0.5,175,8,dmg||9,1.6); }

// --- 패턴 ---
function gp_straight(b){ const pa=Math.atan2(player.y-b.y,player.x-b.x),k=b.enraged?9:7; for(let i=0;i<k;i++) gShot(b.x,b.y,pa+(i-(k-1)/2)*0.16,240,7,12); banner('직선 탄막','',600); }
...
function gp_tongueRush(b){ const pa=Math.atan2(player.y-b.y,player.x-b.x); for(let i=-2;i<=2;i++) gShot(b.x,b.y,pa+i*0.12,300,12,15,0.4); gRing(b,b.enraged?18:14,155,12); banner('혀 내밀기','정면을 비워라',850); }
function gp_totalCollapse(b){ if(sfx.enemyGlitch) sfx.enemyGlitch(); GL.blackout=Math.max(GL.blackout||0,0.45); gView.rotT+=rand(-0.45,0.45); gView.fxT=Math.random()<0.5?-1:1; GL.rotActive=4.2; gHoming(b,13); gRing(b,b.enraged?18:14,170,13); banner('TOTAL COLLAPSE','방송이 찢어진다',950); }
function gp_buffering(b){ if(sfx.enemyGlitch) sfx.enemyGlitch(); banner('▣ 버퍼링','로딩 링을 피해라',850); const waves=b.enraged?5:4; for(let w=0;w<waves;w++){ setTimeout(()=>{ if(boss!==b) return; const base=b.angle*2.4+w*1.15, k=22, gap=TAU*0.28; for(let i=0;i<k;i++){ const off=i/k*TAU; if(off>TAU-gap) continue; gShot(b.x,b.y,base+off,185,7,12); } }, w*330); } }
...
function pickSeungwooPattern(b,list){
  ...
  const fn=pick(pool);
  b._lastGp2=b._lastGp; b._lastGp=fn; b._lastGpTag=seungwooPatternTag(fn);
  return fn;
}
...
function seungwooNextPhase(b){
  ...
  b.enraged=false; b.usedReset=false; b.patI=0; b._lastGp=null; b._lastGp2=null; b._lastGpTag=null; b._gpRep=0; b.attackT=2.6; b.moveT=0;
...
function updateSeungwoo(b,dt){
  ...
  if(b.attackT<=0){
    ...
    let fn;
    if((b._gpRep||0)>0){ b._gpRep--; fn=b._gpLast; }
    else { fn=pickSeungwooPattern(b,list); b._gpLast=fn; if(SEUNGWOO_REPEAT.has(fn)) b._gpRep=1+(Math.random()<0.5?1:0); }
    fn(b);
    b.patI++;
    b.attackT=b.gphase===1?(b.enraged?2.1:2.6):(b.gphase===2?(b.enraged?2.25:2.8):(b.enraged?2.05:2.45));
    if((b._gpRep||0)>0) b.attackT*=0.5;
  }

  // 분신 사격
  for(const c of gClones){ c.t-=dt; c.fireT-=dt; if(c.fireT<=0){ c.fireT=rand(.7,1.1); const pa=Math.atan2(player.y-c.y,player.x-c.x); gShot(c.x,c.y,pa,210,6,10); } }
...
function drawKeyRevOverlay(){
  const pulse=0.5+0.5*Math.sin((typeof performance!=='undefined'?performance.now():Date.now())/120);
  ctx.save();
  ctx.fillStyle='rgba(120,90,170,0.16)'; ctx.fillRect(0,0,W,H);
  ctx.lineWidth=8; ctx.strokeStyle='rgba(255,77,210,'+(0.32+0.26*pulse)+')'; ctx.strokeRect(4,4,W-8,H-8);
  ctx.lineWidth=3; ctx.strokeStyle='rgba(56,232,255,'+(0.28+0.30*pulse)+')'; ctx.strokeRect(11,11,W-22,H-22);
  ctx.textAlign='center';
  ctx.font='bold 22px sans-serif';
  ctx.fillStyle='rgba(255,77,210,'+(0.55+0.45*pulse)+')';
  ctx.fillText('\u2B83 \uC870\uC791 \uBC18\uC804 \u2014 \uBC29\uD5A5\uC774 \uB4A4\uC9D1\uD614\uB2E4', W/2, 42);
  ctx.textAlign='left'; ctx.restore();
}
...
function update(dt){
    ...
    if(!dead && boss){
      if(!b.hitSet.has(boss) && dist2(b.x,b.y,boss.x,boss.y)<(b.r+boss.r)**2){
        if(b.playerShot && (boss.reflectT||0)>0){
          ...
          eBullets.push({x:boss.x,y:boss.y,vx:Math.cos(ra)*330,vy:Math.sin(ra)*330,r:Math.max(5,Math.min(8,b.r||6)),dmg:rd,life:2.6,srcName:(boss.key==='set3'?'케케로로 역송출':'키죠 반사'),reflected:true});
  ...
  const enemyFrozen=player.timeStop>0||roomPreviewT>0;
  // 적 탄
  if(!enemyFrozen) for(let i=eBullets.length-1;i>=0;i--){
    ...
    if(b._rwFreeze>0){ b._rwFreeze-=dt; }
    else {
      if(b._rwHome){
        const rb=(bagjeinRewind&&bagjeinRewind.e&&enemies.includes(bagjeinRewind.e))?bagjeinRewind.e:null;
        if(rb){ const ang=Math.atan2(rb.y-b.y,rb.x-b.x); const sp=Math.min(Math.hypot(b.vx,b.vy)*1.08+30,520); b.vx=Math.cos(ang)*sp; b.vy=Math.sin(ang)*sp; b.life=Math.max(b.life,0.5); if(dist2(b.x,b.y,rb.x,rb.y)<2600){ eBullets.splice(i,1); continue; } }
        else b._rwHome=false;
      }
      let _ldt=dt;
      if(set3Half){ const slowSide=set3Half.leftSlow?(b.x<set3Half.splitX):(b.x>=set3Half.splitX); _ldt=dt*(slowSide?0.55:1.5); }
      b.x+=b.vx*_ldt; b.y+=b.vy*_ldt; b.life-=_ldt; if(b.spinV) b.spin=(b.spin||0)+b.spinV*_ldt;
    }
  ...
  if(!enemyFrozen) for(const e of enemies){
    ...
    if((e.stunT=(e.stunT||0))>0){ e.stunT-=dt; }
    else if((e.ai==='cleaver_thrower'||e.ai==='boomerang_thrower') && kiterRush(e,a,d,dt)){
      // 안티-터틀 러시 진행 중 — 일반 카이팅 AI 스킵
    }
    else if(e.ai==='chase'){
      ...
      if(e.type==='apple' && (e._rollT||0)>0){
        e._rollT-=dt; e.wob+=dt*16;   // 빠른 회전 = 구르는 느낌
        e.x=clamp(e.x+e._rollAx*e.spd*3.6*dt,e.r,W-e.r);
        e.y=clamp(e.y+e._rollAy*e.spd*3.6*dt,e.r,H-e.r);
        if(e._rollT<=0) e.stunT=Math.max(e.stunT||0,0.3);   // 구른 뒤 짧은 경직
      } else {
      ...
      if(e.coolT<=0){
        ...
        if(e.type==='gwangcheon_gim' && e.shotN%3===0){
          ...
          if(e.shotN%4===0){
            ...
            if(typeof beep==='function')beep(440,0.05,'square',0.03);
          } else {
      ...
      if(e.coolT<=0){
        if(enr){ for(let i=-1;i<=1;i++) fireBoomerang(e,a,{enraged:true,spread:i*0.26}); e.coolT=(e.cool||1.95)*0.78; }
      ...
      e._whirlT=(e._whirlT==null?rand(5,7):e._whirlT)-dt; if(e._whirlT<=0){ summonerSandWhirl(e); e._whirlT=rand(6,8); }
      ...
      if((e.reflectT||0)>0){ e._xbT=(e._xbT==null?0.55:e._xbT)-dt; if(e._xbT<=0){ reflectorCrossBeam(e); e._xbT=0.9; } } else e._xbT=null;
      const target=e.range||360;
      ...
      e._revT=(e._revT==null?rand(4.5,6.5):e._revT)-dt; if(e._revT<=0){ magnetReverseBurst(e); e._revT=rand(5.5,7.5); }
      ...
      if(e.coolT<=0){ const ox=e.x, oy=e.y; e.x=clamp(e.x+rand(-140,140),e.r,W-e.r); e.y=clamp(e.y+rand(-110,110),80,H-55); burst(ox,oy,'#38e8ff',8,120); burst(e.x,e.y,'#ff4dd2',8,120); lagAfterimage(ox,oy,e); spawnWarnedSlowField(player.x,player.y,86,4.2,0.75,e.name||e.label); const pa=Math.atan2(player.y-e.y,player.x-e.x);
      ...
      e._tethT=(e._tethT==null?6:e._tethT)-dt; if(e._tethT<=0&&!a3tether){ onsterTether(e); e._tethT=e.awakened?rand(6,8):rand(8.5,11); }
      ...
      // ── 3막 중간보스 사슬 패턴 (셔플백 + 연속 버스트) ──
      e.a3T=(e.a3T==null?6:e.a3T)-dt;
      if(e.a3T<=0){
        if((e._onRep||0)>0){
          e._onRep--; runOnsterPat(e,e._onPat);
          e.a3T = e._onRep>0 ? a3Jit(ph===2?1.4:1.7,0.1) : a3Jit(ph===2?6.0:6.6,0.12);
        } else {
          const pool=ph===2?ONSTER_PATS_P2:ONSTER_PATS_P1;
          const p=pool[nextFromBag(e,ph===2?'_onBag2':'_onBag1',pool.length)];
          e._onPat=p; runOnsterPat(e,p);
          if(p==='whip'||p==='grid'||(ph===2&&p==='burst')){ e._onRep=2; e.a3T=a3Jit(ph===2?1.4:1.7,0.1); }
          else e.a3T=a3Jit(ph===2?6.0:6.6,0.12);
        }
      }
      ...
      if(e.atkT<=0){
        ...
        if(ph===1){
          ...
          const slot=e.atkN%4;
          if(slot===0){ const gx=clamp(player.x+rand(-50,50),80,W-80), gy=clamp(player.y+rand(-40,40),100,H-80); warnAoE(gx,gy,95,0.70,0.4,16,'온스터 지면강타','#8d72ff'); if(typeof beep==='function')beep(110,0.12,'sawtooth',0.05); }
          ...
          if(e.atkN%4===0){ const pa=Math.atan2(player.y-e.y,player.x-e.x); for(let i=-2;i<=2;i++) eBullets.push({x:e.x,y:e.y,vx:Math.cos(pa+i*0.22)*255,vy:Math.sin(pa+i*0.22)*255,r:8,dmg:15,life:3.5,home:0.55,srcName:'\uC628\uC2A4\uD130 \uC720\uB3C4 \uD30C\uD3B8'}); }
      ...
      e.wob+=dt*3;
      if(e._hjFake){
        e.fuseT=Math.max(e.fuseT||0,2);
        if((e._hjCtr||0)>0) e._hjCtr-=dt;
        if(e.hp<(e._hjHp!=null?e._hjHp:e.hp)){
          e.hp=e.maxhp;
          if((e._hjCtr||0)<=0){ e._hjCtr=0.9; const pa=Math.atan2(player.y-e.y,player.x-e.x); for(let i=-1;i<=1;i++) eBullets.push({x:e.x,y:e.y,vx:Math.cos(pa+i*0.2)*240,vy:Math.sin(pa+i*0.2)*240,r:8,dmg:12,life:3.2,srcName:'가짜 코어 반격'}); burst(e.x,e.y,'#ff4d6d',10,180); if(typeof beep==='function')beep(90,0.12,'square',0.05); banner('✖ 가짜 코어!','반격탄',700); }
        }
        e._hjHp=e.hp;
      }
      e.fuseT-=dt;
      ...
      if(e.fuseT<=0){
        ...
        if(e._onExpire){ const cb=e._onExpire; e._onExpire=null; try{cb(e);}catch(_){} }
        const ix=enemies.indexOf(e); if(ix>=0) enemies.splice(ix,1);
        continue;
      }
    }
      ...
      if(e._idLife){ e._fireT=(e._fireT==null?1.2:e._fireT)-dt; if(e._fireT<=0){ const pa=Math.atan2(player.y-e.y,player.x-e.x); for(let i=-1;i<=1;i++) eBullets.push({x:e.x,y:e.y,vx:Math.cos(pa+i*0.2)*215,vy:Math.sin(pa+i*0.2)*215,r:7,dmg:11,life:3.2,srcName:'현진 검기',col:'#38e8ff'}); e._fireT=rand(1.0,1.6); } }
      if(e._ctrCd>0) e._ctrCd-=dt;
      if(e.hp < (e._lastHp||e.hp)){
        e._lastHp=e.hp=999999;                       // 가짜는 항상 무적 유지
        if((e._ctrCd||0)<=0){                        // 반격탄은 쿨다운 게이트 — 연사/관통 스팸 방지
          const pa=Math.atan2(player.y-e.y,player.x-e.x);
          for(let i=-1;i<=1;i++) eBullets.push({x:e.x,y:e.y,vx:Math.cos(pa+i*0.2)*240,vy:Math.sin(pa+i*0.2)*240,r:8,dmg:12,life:3.2,srcName:'현진 반격'});
          burst(e.x,e.y,'#ff4d6d',10,180); beep(90,0.12,'square',0.05); banner('✖ 가짜!','반격탄',700);
          e._ctrCd=0.9;
        }
      }
      ...
      if(e.climaxT>0){
        ...
        if(e.atkT<=0){
          ...
          if((e.atkRep||0)<=0 && ph>=2 && (e._tideCd=(e._tideCd||0)-1)<=0 && !hazards.some(h=>h.kind==='creepTide') && Math.random()<0.28){
            hyechulCreepTide(e,ph); e._tideCd=4;
            e.atkT=ph>=3?1.4:1.6;
          } else if((e.atkRep||0)<=0 && ph>=3 && (e._slamCd=(e._slamCd||0)-1)<=0 && Math.random()<0.34){
            ...
            e.atkT=1.0;
          } else if((e.atkRep||0)<=0 && (e._rushCd=(e._rushCd||0)-1)<=0 && enemies.filter(o=>o.type==='zergling'||o.type==='mutalisk'||o.type==='ultra').length>=3 && Math.random()<0.45){
            hyechulSummonRush(e); e._rushCd=2;
            if(typeof beep==='function')beep(150,0.25,'sawtooth',0.06); screenShake=Math.max(screenShake||0,5);
            e.atkT=ph===1?1.6:(ph===2?1.4:1.2);
          } else {
      ...
      if(e.cs==='approach'){
        ...
        if(e.coolT<=0 && d<470){
          if(e.type==='rhino_beetle'){
            ...
            if(worms<6 && roll<0.20){ e.cs='cast'; e.csT=0.6; }
            else if(roll<0.42){ e.cs='spit'; e.csT=0.35; e.spitRep=3+(Math.random()<0.5?1:0); e.aimX=Math.cos(a); e.aimY=Math.sin(a); }
            else if(roll<0.61){ e.cs='burst'; e.csT=0.55; e.burstRep=3+(Math.random()<0.5?1:0); }
            else if(roll<0.74){ e.cs='emote'; e.csT=0.9; e._emoteN=18; e._emoteGap=3+(Math.random()<0.5?1:0); e._emoteGs=Math.floor(Math.random()*18); }
        ...
        e.csT-=dt; e.wob+=dt*28; e.faceAng=Math.atan2(e.aimY,e.aimX);
        if(e.csT<=0){ e.cs='dash'; e.csT=0.50; playPatternSfx('enemyDash2',()=>{ beep(180,0.08,'sawtooth',0.05); setTimeout(()=>beep(330,0.045,'square',0.03),45); },0.22); }
      } else if(e.cs==='dash'){
        ...
        if(e.csT<=0 && !e._bounced){ e._bounced=false; if(e.type==='rhino_beetle'&&e.hp<=e.maxhp*0.55&&!e._doubleDashUsed&&Math.random()<0.65){ e._doubleDashUsed=true; e.cs='reaim'; e.csT=rand(0.5,0.6); e.coolT=0; { const ra=Math.atan2(player.y-e.y,player.x-e.x); e.aimX=Math.cos(ra); e.aimY=Math.sin(ra); } burst(e.x,e.y,'#ffd34d',10,160); } else { e.cs='rest'; e.csT=0.6; e.coolT=1.7; if(e.type==='rhino_beetle') e._intentChargeCd=Math.max(e._intentChargeCd||0,3.5); } }
        if(e.csT<=0 && e._bounced){ e._bounced=false; e.cs='rest'; e.csT=0.6; e.coolT=1.7; }
      } else if(e.cs==='emote'){
        // D. 긁기 — 가장자리에서 빈틈을 두고 플레이어로 수렴 (빈틈/위험 링은 차징 중 텔레그래프로 예고)
        e.csT-=dt; e.wob+=dt*20; e.faceAng=a;
        if(e.csT<=0){
          const N=e._emoteN||18, gap=e._emoteGap||3, gs=(e._emoteGs==null?0:e._emoteGs);
          const px=player.x, py=player.y;
          for(let i=0;i<N;i++){
            if(i>=gs && i<gs+gap) continue;   // 예고된 빈틈(회피 경로)
            const a2=i/N*TAU, sx=px+Math.cos(a2)*330, sy=py+Math.sin(a2)*330;   // clamp 제거: 구석에서 탄 뭉침 방지(빈틈 구조 보존)
            const ia=Math.atan2(py-sy,px-sx);
            eBullets.push({x:sx,y:sy,vx:Math.cos(ia)*180,vy:Math.sin(ia)*180,r:7,dmg:16,life:3.4,srcName:(e.name||e.label)+' 긁기',col:'#9146ff'});
          }
          burst(player.x,player.y,'#9146ff',14,200); banner('긁혔다!','',650);
          if(typeof beep==='function')beep(360,0.08,'square',0.05);
          e._emoteGs=null; e._emoteGap=null;
          e.cs='rest'; e.csT=0.5; e.coolT=1.4;
        }
      ...
      if(e.climaxT>0){
        ...
        if(e.climaxT<=0){
          ...
          for(let ring=0;ring<3;ring++){
            ...
            for(let i=0;i<k;i++){ const a2=i/k*TAU+ring*0.28+(e.atkN||0)*0.7; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*sp,vy:Math.sin(a2)*sp,r:9,dmg:18,life:4.2,srcName:'양갱',kkot:true}); }
          }
        ...
        if(e.atkT<=0){
          const pa=Math.atan2(player.y-e.y,player.x-e.x);
          let slot;
          if((e._kkRep||0)>0){ e._kkRep--; slot=e._kkSlot; }
          else { slot=nextFromBag(e,'_kkBag',6); e._kkSlot=slot; const _ph=e.phase||1; const _homing=(_ph===2&&slot===1)||(_ph===3&&slot===0); if((slot===0||slot===1)&&!_homing) e._kkRep=2+(Math.random()<0.5?1:0); }
          e.atkN=(e.atkN||0)+1;
          ...
          if(ph===1){
            if(slot===0){
              // 꽃잎 소용돌이 8발 (회전 오프셋)
              const k=8; for(let i=0;i<k;i++){ const a2=i/k*TAU+e.atkN*0.35; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*215,vy:Math.sin(a2)*215,r:8,dmg:11,life:3.2,srcName:'양갱',kkot:true}); }
              banner('🍯 꽃잎 사격','',500); beep(880,0.05,'sine',0.04);
              e.atkT=base;
            } else if(slot===1){
              // 조준 3연사
              for(let i=0;i<3;i++) setTimeout(()=>{ if(enemies.includes(e)){ const a2=Math.atan2(player.y-e.y,player.x-e.x); eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*265,vy:Math.sin(a2)*265,r:8,dmg:12,life:3.0,srcName:'양갱',kkot:true}); } },i*130);
              e.atkT=base;
            } else if(slot===2){
              // 바닥 가시 (플레이어 위치 + 랜덤 8곳)
              ...
              // 3발 부채꼴
              for(let i=-1;i<=1;i++) eBullets.push({x:e.x,y:e.y,vx:Math.cos(pa+i*0.28)*240,vy:Math.sin(pa+i*0.28)*240,r:9,dmg:12,life:3.5,srcName:'양갱',kkot:true});
              e.atkT=base;
            } else if(slot===4){
              // 덩굴 슬로우장
              ...
              for(let i=0;i<rainN;i++){ const fx=rand(20,W-20); eBullets.push({x:fx,y:-12,vx:rand(-15,15),vy:rand(170,210),r:8,dmg:11,life:4.5,srcName:'양갱',kkot:true}); }
            ...
            if(slot===0){
              // 소용돌이 12발 (격노 링)
              const k=12; for(let i=0;i<k;i++){ const a2=i/k*TAU+e.atkN*0.4; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*220,vy:Math.sin(a2)*220,r:8,dmg:13,life:3.6,srcName:'양갱',kkot:true}); }
              banner('🍯 꽃잎 링','전방위 사격!',600);
              e.atkT=base;
            } else if(slot===1){
              // 눈알 추적탄 4발
              const k=4; for(let i=0;i<k;i++){ const a2=pa+(i-(k-1)/2)*0.22; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*240,vy:Math.sin(a2)*240,r:9,dmg:14,life:3.4,srcName:'양갱',kkot:true,home:1.6}); }
              ...
              // 더블 링 (혜철이 포자링 스타일)
              const k=12;
              for(let i=0;i<k;i++){ const a2=i/k*TAU+(e.atkN||0)*0.7; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*195,vy:Math.sin(a2)*195,r:8,dmg:13,life:3.8,srcName:'양갱',kkot:true}); }
              for(let i=0;i<k;i++){ const a2=i/k*TAU+Math.PI/k+(e.atkN||0)*0.7; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*235,vy:Math.sin(a2)*235,r:7,dmg:12,life:3.4,srcName:'양갱',kkot:true}); }
              e.atkT=base;
            } else if(slot===4){
              // 꽃잎 비 (측면도 추가)
              const rainN=22+irand(0,6);
              for(let i=0;i<rainN;i++){ const fx=rand(15,W-15); eBullets.push({x:fx,y:-12,vx:rand(-18,18),vy:rand(178,218),r:8,dmg:13,life:4.5,srcName:'양갱',kkot:true}); }
              for(let i=0;i<4;i++){ eBullets.push({x:-10,y:rand(100,H-100),vx:rand(135,175),vy:rand(-18,18),r:7,dmg:12,life:4,srcName:'양갱',kkot:true}); eBullets.push({x:W+10,y:rand(100,H-100),vx:-rand(135,175),vy:rand(-18,18),r:7,dmg:12,life:4,srcName:'양갱',kkot:true}); }
            ...
            if(slot===0){
              // 눈알 추적 5발 + 소용돌이
              const k=5; for(let i=0;i<k;i++){ const a2=pa+(i-(k-1)/2)*0.2; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*255,vy:Math.sin(a2)*255,r:9,dmg:16,life:3.2,srcName:'양갱',kkot:true,home:2.0}); }
              const k2=10; for(let i=0;i<k2;i++){ const a2=i/k2*TAU+e.atkN*0.5; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*195,vy:Math.sin(a2)*195,r:7,dmg:13,life:3.8,srcName:'양갱',kkot:true}); }
              ...
              // 트리플 링
              const k=14;
              for(let ring=0;ring<3;ring++){ for(let i=0;i<k;i++){ const a2=i/k*TAU+ring*(Math.PI/k)+(e.atkN||0)*0.7; eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*(185+ring*38),vy:Math.sin(a2)*(185+ring*38),r:8,dmg:15,life:4.0,srcName:'양갱',kkot:true}); } }
              banner('🍯 트리플 링','전방위 최강!',750);
              e.atkT=base;
            } else if(slot===3){
              // 꽃잎 비 최강 + 사방
              const rainN=32+irand(0,8);
              for(let i=0;i<rainN;i++){ const fx=rand(10,W-10); eBullets.push({x:fx,y:-12,vx:rand(-22,22),vy:rand(182,235),r:9,dmg:15,life:4.8,srcName:'양갱',kkot:true}); }
              for(let i=0;i<7;i++){ eBullets.push({x:-10,y:rand(70,H-70),vx:rand(148,195),vy:rand(-22,22),r:7,dmg:14,life:4,srcName:'양갱',kkot:true}); eBullets.push({x:W+10,y:rand(70,H-70),vx:-rand(148,195),vy:rand(-22,22),r:7,dmg:14,life:4,srcName:'양갱',kkot:true}); }
          ...
          beep(120,0.08,'sawtooth',0.04); if((e._kkRep||0)>0) e.atkT=base*0.55;
        }
      }
    }
      ...
      // [시그니처] 되감기 — 격노(B면) 직전 마지막 발악 1회
      if(!e._rewindDone && !e.enr && e.hp < e.maxhp*0.46){ e._rewindDone=true; bagjeinDoRewind(e); }
      ...
      if(e.enr){
        ...
        if(e.scratchT<=0){
          ...
          const k=32; for(let i=0;i<k;i++){
            ...
            eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*285,vy:Math.sin(a2)*285,r:7,dmg:16,life:4.8,srcName:'박제인간'});
          }
          banner('💿 레코드 긁힘!','…',700);
      ...
      if(e.enr){
        ...
        if(e.rotHackT<=0){
          ...
          const k=14; for(let i=0;i<k;i++){
            const a2=i/k*TAU+e.spinAng;
            eBullets.push({x:e.x,y:e.y,vx:Math.cos(a2)*220,vy:Math.sin(a2)*220,r:8,dmg:14,life:3.8,srcName:'박제인간'});
          }
        }
      }

      // [특수4] 스크래치 디제잉 — 격노 후 13초마다 (좌우 횡 스윕 벽, 틈으로 회피)
      if(e.enr){
        e.djT=(e.djT==null?13:e.djT)-dt;
        if(e.djT<=0){
          e.djT=13;
          banner('\uD83D\uDCBF \uC2A4\uD06C\uB798\uCE58!','\uBCBD\uC774 \uAE41\uACE0 \uC9C0\uB098\uAC04\uB2E4',850);
      ...
      if(e.atkT<=0){
        ...
        const en=e.enr;
        let ph;
        if((e._bagRep||0)>0){ e._bagRep--; ph=e._bagPh; }
        else { ph=nextFromBag(e,'_bagBag',7); e._bagPh=ph; if(ph===0||ph===1||ph===6) e._bagRep=2+(Math.random()<0.5?1:0); }
        ...
        if((e._bagRep||0)>0) e.atkT=en?0.85:1.25;
      }
    }
...
function onCombatCleared(){
  ...
  setTimeout(()=>{
    ...
    else if(roomHadElite){ const bonus=irand(80,125); addGold(bonus,'roomReward'); updateHUD(); banner(eliteRewardTitle(currentEliteKind()),'골드 +'+bonus,1400); finishNode(); }
...
function rbA2Build(c, glows, parts, role){
  const P=RB_P;
  // 2막: 황혼 + 디지털 글리치 침투. 방마다 색조·구성·포인트를 분명히 달리함.
  if(role==='elite'){
    // 정예 — 깊은 석양, 지는 해(중앙), 관문·석탑, 맹금, 강한 글리치
    rbInkBase(c, glows, { moon:false, skyTop:'#321816', skyMid:'#1e0e10', skyBot:'#120809',
      mtn:['#301818','#241012','#1a0c0e','#120809'], fog:50, fogA:0.08, fogCol:'210,120,70' });
    rbInkSun(c, glows, Math.round(W*0.5), Math.round(H*0.34), 58, '230,100,55');
    rbInkNeonVein(c, W*0.10, H*0.46, 200, '255,80,180'); rbInkNeonVein(c, W*0.62, H*0.52, 240, '70,230,210');
    rbInkGate(c, Math.round(W*0.5), Math.round(H*0.80), 160, Math.round(H*0.42), '#1a0e0c');
    rbInkPagoda(c, 120, Math.round(H*0.86), Math.round(H*0.46), '#170c0e');
    rbInkPagoda(c, W-130, Math.round(H*0.88), Math.round(H*0.40), '#170c0e');
    for(let i=0;i<4;i++) rbInkHawk(c, W*0.2+rbHash(i,71)*W*0.6, H*0.18+rbHash(i,72)*H*0.18, 6+(rbHash(i,73)*4|0)*P);
    rbInkGlitchCrack(c, W*0.3, H*0.25, 110, true, '255,80,180'); rbInkGlitchCrack(c, W*0.8, H*0.35, 90, false, '70,230,210');
    for(let i=0;i<5;i++) glows.push({x:rbRnd()*W,y:60+rbRnd()*(H-120),r:26,col:'255,90,70',base:0,amp:0.2,s:i*2,warn:1});
  }
  else if(role==='mid'){
    // 중간보스(박제인간) — 멈춘 시간: 탁한 회보라, 짙은 안개, 박제 진열장, 정지된 나비, 차가운 달
    rbInkBase(c, glows, { moonCol:'160,140,150', moonX:W*0.70, moonY:H*0.18, moonR:30,
      skyTop:'#221a22', skyMid:'#161018', skyBot:'#0c0810',
      mtn:['#221c24','#181420','#120e18','#0a0810'], fog:74, fogA:0.14, fogCol:'170,150,170' });
    rbInkCaseRow(c, glows);
    for(let i=0;i<6;i++){ const bx=W*0.18+rbHash(i,81)*W*0.64, by=H*0.30+rbHash(i,82)*H*0.18; rbInkButterfly(c, bx, by, i%2?'#3a2430':'#34202a'); }
    rbInkGlitchCrack(c, W*0.25, H*0.30, 60, true, '120,200,210');
    rbInkFog(c, H*0.50, 90, 0.10, '160,150,175');
    glows.push({x:W*0.5,y:H*0.45,r:120,col:'180,150,170',base:0.05,amp:0.05,s:1});
  }
  else if(role==='boss'){
    // 최종보스(승우) — 수묵이 디지털로 붕괴: 가장 어둡고 분열, 깨지는 해, 디지털 격자, 글리치 폭주
    rbInkBase(c, glows, { moon:false, skyTop:'#1c1418', skyMid:'#120c12', skyBot:'#08060a',
      mtn:['#1c1620','#16101a','#100c14','#08060a'], fog:46, fogA:0.06, fogCol:'120,90,160' });
    rbInkSun(c, glows, Math.round(W*0.5), Math.round(H*0.24), 50, '200,70,90');
    rbInkGrid(c, Math.round(H*0.58), '70,230,210');
    for(let i=0;i<9;i++) rbInkGlitchCrack(c, rbRnd()*W, rbRnd()*H*0.7, 70+rbRnd()*90, rbRnd()>0.5, i%2?'70,230,210':'255,80,180');
    rbInkPagoda(c, 140, Math.round(H*0.88), Math.round(H*0.4), '#140c12'); rbInkPagoda(c, W-150, Math.round(H*0.9), Math.round(H*0.36), '#140c12');
    rbGlitchBand(c);
    glows.push({x:W/2,y:H*0.4,r:170,col:'180,70,180',base:0.05,amp:0.09,s:1});
  }
  else {
    // 일반 — 따뜻한 황혼 단풍 산수(가장 평온), 노을 해(우측), 글리치 약간
    rbInkBase(c, glows, { moon:false, skyTop:'#2a1c1c', skyMid:'#1c1214', skyBot:'#100a0c',
      mtn:['#2a1e1e','#221618','#180f12','#100a0c'], fog:54, fogA:0.09, fogCol:'205,150,95' });
    rbInkSun(c, glows, Math.round(W*0.74), Math.round(H*0.24), 44, '224,135,72');
    rbInkNeonVein(c, W*0.10, H*0.44, 160, '70,230,210');
    rbInkMaple(c, 90, Math.round(H*0.88), Math.round(H*0.40), '#1a0e0a', '#7a241a');
    rbInkMaple(c, W-110, Math.round(H*0.90), Math.round(H*0.34), '#1a0e0a', '#6e2218');
    rbInkPagoda(c, 210, Math.round(H*0.86), Math.round(H*0.42), '#170e10');
    rbInkGlitchCrack(c, W*0.84, H*0.40, 60, false, '255,80,180');
  }
}
// 3막 방 빌더
...
/* ===== 2막 황혼 수묵 + 디지털 글리치 침투 헬퍼 ===== */
// 노을 해 (붉은 원반 + 펄스 글로우)
function rbInkSun(c, glows, x, y, r, col){ rbInkMoon(c, x, y, r, col); glows.push({x:x,y:y,r:r*2.2,col:col,base:0.07,amp:0.06,s:2}); }
// 단풍나무 (굽은 줄기 + 붉은 잎 덩어리)
function rbInkMaple(c, x, groundY, h, col, leaf){
  const P=RB_P; x=Math.round(x/P)*P; col=col||'#1a0e0a'; leaf=leaf||'#5a1e16'; let tx=x; const segs=Math.round(h*0.6/P);
  for(let i=0;i<segs;i++){ const t=i/segs; tx=x+Math.sin(t*1.6+x)*h*0.06*t; c.fillStyle=col; c.fillRect(Math.round(tx/P)*P, groundY-i*P, P*(t<0.5?2:1), P); }
  const topY=groundY-h*0.55;
  for(let k=0;k<14;k++){ const a=k/14*Math.PI*2, rr=h*0.3*(0.6+rbHash(x+k,1)*0.4), lx=tx+Math.cos(a)*rr, ly=topY-Math.abs(Math.sin(a))*rr*0.7-h*0.1;
    c.fillStyle=(k%3?leaf:'#6e2820'); c.fillRect(Math.round(lx/P)*P, Math.round(ly/P)*P, P*2, P*2); }
}
// 석탑/탑 (동양 건축, 층층 처마)
function rbInkPagoda(c, x, groundY, h, col){
  const P=RB_P; x=Math.round(x/P)*P; col=col||'#150d10'; const floors=4, fh=h/floors;
  for(let f=0;f<floors;f++){ const t=f/floors, w=(h*0.4)*(1-t*0.6), y=groundY-h*0.95+f*fh*0.9;
    c.fillStyle=col; c.fillRect(Math.round((x-w/2)/P)*P, Math.round((y+fh*0.3)/P)*P, Math.max(P,Math.round(w/P)*P), Math.max(P,Math.round(fh*0.6/P)*P));
    c.fillStyle='#0a0608'; c.fillRect(Math.round((x-w*0.62)/P)*P, Math.round((y+fh*0.15)/P)*P, Math.round(w*1.24/P)*P, P*2); }
  c.fillStyle=col; c.fillRect(x-P, groundY-h, P*2, P*4);
}
// 디지털 글리치 균열 (수묵 위 네온 지그재그 + 변위 띠)
function rbInkGlitchCrack(c, x, y, len, vert, col){
  const P=RB_P; let cx=Math.round(x/P)*P, cy=Math.round(y/P)*P; col=col||'70,230,210';
  for(let i=0;i<len/P;i++){ c.fillStyle='rgba('+col+',0.5)'; c.fillRect(cx,cy,P,P);
    if(rbHash(cx,cy)>0.7){ c.fillStyle='rgba('+col+',0.14)'; c.fillRect(cx-P*2, cy, P*4, P); }
    if(vert){ cy+=P; if(rbHash(cx,cy)>0.6) cx+=(rbHash(cy,cx)>0.5?P:-P); }
    else { cx+=P; if(rbHash(cy,cx)>0.6) cy+=(rbHash(cx,cy)>0.5?P:-P); } }
}
// 산 능선의 네온 실금 (디지털 침투의 흔적)
function rbInkNeonVein(c, x, y, w, col){
  const P=RB_P; col=col||'255,80,180';
  for(let i=0;i<w/P;i++){ const xx=Math.round(x/P)*P+i*P, yy=Math.round(y/P)*P+Math.round(Math.sin(i*0.3)*2)*P; if(rbHash(xx,yy)>0.3){ c.fillStyle='rgba('+col+',0.28)'; c.fillRect(xx,yy,P,P);} }
}
// 박제 진열장 (박제인간 — 멈춘 시간, 수묵 톤)
function rbInkCaseRow(c, glows){
  const P=RB_P;
  [180,400,620].forEach((x,i)=>{ const top=Math.round(H*0.40), h=Math.round(H*0.32), w=P*20;
    c.fillStyle='#140d10'; c.fillRect(x, top, w, h); c.fillStyle='#1e161a'; c.fillRect(x+P, top+P, w-P*2, h-P*2);
    c.fillStyle='rgba(220,180,140,0.04)'; c.fillRect(x+P, top+P, w-P*2, P);
    c.fillStyle='#0c0809'; c.fillRect(Math.round(x+w*0.35), Math.round(top+h*0.4), Math.round(w*0.3), Math.round(h*0.5));
    glows.push({x:x+w/2,y:top+h*0.4,r:14,col:'200,150,90',base:0.05,amp:0.06,s:i}); });
}
// 동양 성문/관문 (기와지붕 + 통로)
function rbInkGate(c, x, groundY, w, h, col){
  const P=RB_P; x=Math.round(x/P)*P; col=col||'#1a0e0c';
  c.fillStyle=col; c.fillRect(Math.round((x-w/2)/P)*P, Math.round((groundY-h*0.5)/P)*P, Math.round(w/P)*P, Math.round(h*0.5/P)*P);
  c.fillStyle='#080405'; c.fillRect(Math.round((x-w*0.15)/P)*P, Math.round((groundY-h*0.5)/P)*P, Math.round(w*0.3/P)*P, Math.round(h*0.5/P)*P);
  const rh=Math.round(h*0.3/P);
  for(let i=0;i<rh;i++){ const t=i/rh, rw=w*(1.2-t*0.5); c.fillStyle=i<2?'#0c0608':'#241410'; c.fillRect(Math.round((x-rw/2)/P)*P, Math.round((groundY-h*0.5)/P)*P-i*P, Math.round(rw/P)*P, P); }
  c.fillStyle='#2a1812'; c.fillRect(Math.round((x-w*0.6)/P)*P, Math.round((groundY-h*0.5)/P)*P-rh*P, Math.round(w*1.2/P)*P, P*2);
}
// 맹금 (큰 V자 실루엣)
function rbInkHawk(c, x, y, s){ const P=RB_P; x=Math.round(x/P)*P;y=Math.round(y/P)*P; const wy=y-Math.round(s*0.6/P)*P;
  c.fillStyle='rgba(20,12,10,0.8)'; c.fillRect(x,y,P*2,P); c.fillRect(x-s,wy,s,P); c.fillRect(x+P*2,wy,s,P); c.fillRect(x-s-P,wy+P,P*2,P); c.fillRect(x+s+P,wy+P,P*2,P); }
// 정지된 나비 (멈춘 시간)
function rbInkButterfly(c, x, y, col){ const P=RB_P; x=Math.round(x/P)*P;y=Math.round(y/P)*P; col=col||'#3a2430';
  c.fillStyle=col; c.fillRect(x-P*2,y-P,P*2,P*3); c.fillRect(x+P,y-P,P*2,P*3); c.fillStyle='#0c0608'; c.fillRect(x,y,P,P*3); }
// 디지털 원근 격자 (바닥에서 솟는 침투)
function rbInkGrid(c, y0, col){ const P=RB_P; col=col||'70,230,210'; y0=Math.round(y0/P)*P;
  for(let i=0;i<8;i++){ const y=y0+i*i*3; if(y>H)break; c.fillStyle='rgba('+col+','+(0.12-i*0.012).toFixed(3)+')'; c.fillRect(0,Math.round(y/P)*P,W,P); }
  for(let i=-6;i<=6;i++){ c.fillStyle='rgba('+col+',0.05)'; const xb=Math.round((W/2+i*70)/P)*P; c.fillRect(xb,y0,P,H-y0); }
}
// ===== /2막 헬퍼 =====
...
function rbInkFx(c, theme, t){
  ...
  else if(theme.indexOf('a2_')===0){
    const boss=(theme==='a2_boss'), mid=(theme==='a2_mid');
    // 떨어지는 단풍 (중보스/보스는 적게 — 멈춘 시간/붕괴)
    const leaves = mid?2:(boss?3:7);
    for(let i=0;i<leaves;i++){ const y=(H*0.1+t*0.012*(1+i*0.2)+i*100)%H, x=(i*140+40+Math.sin(t*0.0013+i)*36)%W;
      c.fillStyle=(i%2?'rgba(150,50,40,0.5)':'rgba(190,80,50,0.45)'); c.fillRect(x|0,y|0,P*2,P); c.fillRect((x|0)+(Math.sin(t*0.004+i)>0?P:0),(y|0)+P,P,P); }
    // 글리치 깜빡 (보스 폭주 / 중보스는 거의 없음)
    const gth = boss?0.6:(mid?0.985:0.94);
    if(Math.sin(t*0.003)>gth){ const gy=(Math.abs(Math.sin(t*0.04))*H)|0, gh=P*(1+(Math.abs(Math.cos(t*0.06))*(boss?9:5)|0)), sh=(Math.sin(t*0.2)*(boss?26:16)|0)*P;
      c.fillStyle=(Math.sin(t)>0?'rgba(70,230,210,0.14)':'rgba(255,80,180,0.12)'); c.fillRect(sh,gy,W,gh);
      if(boss){ c.fillStyle='rgba(255,80,180,0.08)'; c.fillRect(sh+P*3,gy,W,gh); } }   // 보스: RGB 분열
    // 네온 입자 부유 (보스 더 많이)
    const np = boss?8:5;
    for(let i=0;i<np;i++){ const x=(W*0.2+i*120+Math.sin(t*0.0007+i)*50)%W, y=H*0.45+Math.sin(t*0.001+i*2)*40, a=0.12+0.18*Math.sin(t*0.005+i);
      if(a>0.06){ c.fillStyle=(i%2?'rgba(70,230,210,':'rgba(255,80,180,')+a.toFixed(3)+')'; c.fillRect(x|0,y|0,P,P); } }
  }
}
// ===== /1막 수묵 산수화 =====
...
function drawRoomBg(theme){
  ...
  const _inkTheme=(theme==='forest'||theme==='server'||theme==='hatch'||theme==='food'||theme.indexOf('a2_')===0); // 1·2막 수묵 계열
  if(_inkTheme) rbInkFx(ctx, theme, t);   // 달/해 펄스·안개·물·별·단풍·글리치
...
function drawEnemy(e){
  ...
  if((e._armorBuff||0)>0&&e.type!=='namu'){ ctx.save(); ctx.globalAlpha=0.45+0.3*Math.abs(Math.sin((e.wob||0)*4)); ctx.strokeStyle='#5fa84a'; ctx.lineWidth=2.5; ctx.setLineDash([5,4]); ctx.beginPath(); ctx.arc(0,0,e.r+8,0,TAU); ctx.stroke(); ctx.setLineDash([]); ctx.restore(); }
  if(e.type==='namu'&&(e._namuBuffT||0)>0){ ctx.save(); ctx.globalAlpha=0.4+0.35*Math.abs(Math.sin((e.wob||0)*3)); ctx.strokeStyle='#8fe06a'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(0,0,e.r+12+2*Math.sin((e.wob||0)*3),0,TAU); ctx.stroke(); ctx.globalAlpha=0.9; ctx.font='bold 13px sans-serif'; ctx.textAlign='center'; ctx.fillText('🛡',0,-e.r-15); ctx.textAlign='left'; ctx.restore(); }
  ...
  if(e.ai==='charge' && (e.cs==='wind'||e.cs==='spit'||e.cs==='reaim')){
    const len=(e.cs==='wind'||e.cs==='reaim')?135:78;
  ...
  } else if(e.ai==='charge' && (e.cs==='cast'||e.cs==='burst'||e.cs==='emote')){
    const col=e.cs==='burst'?'202,161,74':(e.cs==='emote'?'145,70,255':'232,122,138');
    ...
    if(e.cs==='emote' && e._emoteGs!=null){
      // 긁기 텔레그래프: 플레이어 주변 위험 링(보라) + 안전 빈틈(초록) — 좌표계 원점이 적 중심이라 플레이어 상대좌표 사용
      const N=e._emoteN||18, gap=e._emoteGap||3, gs=e._emoteGs, RR=330;
      const px=player.x-e.x, py=player.y-e.y, prog=clamp(1-(e.csT||0)/0.9,0,1);
      ctx.save();
      ctx.lineWidth=4; ctx.setLineDash([7,7]); ctx.strokeStyle='rgba(145,70,255,'+(0.28+0.42*prog)+')';
      for(let i=0;i<N;i++){ if(i>=gs&&i<gs+gap) continue; const a0=i/N*TAU, sp=TAU/N*0.42; ctx.beginPath(); ctx.arc(px,py,RR,a0-sp,a0+sp); ctx.stroke(); }
      ctx.setLineDash([]); ctx.lineWidth=5; ctx.strokeStyle='rgba(93,255,155,'+(0.5+0.45*prog)+')';
      ctx.beginPath(); ctx.arc(px,py,RR,(gs/N)*TAU,((gs+gap)/N)*TAU); ctx.stroke();
      const ga=((gs+gap/2)/N)*TAU;
      ctx.lineWidth=3; ctx.setLineDash([5,9]); ctx.strokeStyle='rgba(93,255,155,'+(0.22+0.34*prog)+')';
      ctx.beginPath(); ctx.moveTo(px,py); ctx.lineTo(px+Math.cos(ga)*RR,py+Math.sin(ga)*RR); ctx.stroke();
      ctx.restore();
    }
...
function drawEliteIntro(){
  ...
  if(E.banner>0){
    ...
    ctx.textAlign='center';
    ctx.fillStyle=col; ctx.font='bold 14px sans-serif'; ctx.fillText((isKkot?'🍯 ':isTruck?'🚚 ':'⚠ ')+eliteDisplayName(kind), W/2, by+25);
    ctx.fillStyle='#fff'; ctx.font='bold 19px sans-serif'; ctx.fillText(isKkot?'"꽃이라고 얕봤지? 이제 진짜야."':(isTruck?'"송출 시작."':'"나는야 너무 멋진 장수풍뎅이"'), W/2, by+52);
    ctx.restore();
  }
}
  ...
  for(let i=hazards.length-1;i>=0;i--){ const h=hazards[i]; h.t+=dt;
    if(h.kind==='beamSweep'){
      ...
      if(active&&!done){
        ...
        if(along>0&&along<(h.range||620)&&side<(h.width||18)&&!(h.hitCd>0)){ hurtPlayer(h.dmg||10,h.srcName||'노잭'); h.hitCd=0.35; }
      }
    ...
    if(h.kind==='megabeam'){
      if(h.hitCd>0) h.hitCd-=dt;
      const active=h.t>=h.warnT, done=h.t>=h.warnT+h.liveT;
      if(active&&!h._sfx){ h._sfx=true; if(sfx.enemyLaser) sfx.enemyLaser(); screenShake=Math.max(screenShake||0,7); }
      if(active&&!done){
        const ang=h.ang+(h.t-h.warnT)*(h.rot||0), px=player.x-h.x, py=player.y-h.y;
        const side=Math.abs(-px*Math.sin(ang)+py*Math.cos(ang));
        if(side<h.width && (player.dodging||0)<=0 && !(h.hitCd>0)){ hurtPlayer(h.dmg||22,h.srcName||'대형빔'); h.hitCd=0.35; }
      }
      if(done) hazards.splice(i,1);
      continue;
    }
    if(h.kind==='genpillar'){
      if(h.hitCd>0) h.hitCd-=dt;
      const active=h.t>=h.warnT, done=h.t>=h.warnT+h.liveT;
      if(active&&!h._sfx){ h._sfx=true; if(sfx.enemyExplode) sfx.enemyExplode(); }
      if(active&&!done && Math.abs(player.x-h.x)<h.w/2 && (player.dodging||0)<=0 && !(h.hitCd>0)){ hurtPlayer(h.dmg||20,h.srcName||'제네시스'); h.hitCd=0.4; }
      if(done) hazards.splice(i,1);
      continue;
    }
    if(h.kind==='slowfield'){
    ...
    if(h.kind==='creepTide'){
      const yTop=100, yBot=H-50;
      if(h.t>=h.warnT){
        if(h.phase!=='recede'){
          h.front+=h.dir*h.spd*dt;
          // 선단 근처 플레이어 탄 → 밀어내기
          let push=0;
          for(let bi=0;bi<pBullets.length;bi++){ const b=pBullets[bi]; if(b.y>yTop&&b.y<yBot&&Math.abs(b.x-h.front)<48){ push++; if(push<=8) burst(b.x,b.y,'#d68bff',2,110); } }
          if(push>0) h.front-=h.dir*Math.min(push,10)*140*dt;
          h.front=(h.dir>0)?clamp(h.front,h.edge,h.maxFront):clamp(h.front,h.maxFront,h.edge);
        } else {
          h.front-=h.dir*h.spd*1.7*dt;
          if((h.dir>0&&h.front<=h.edge)||(h.dir<0&&h.front>=h.edge)){ hazards.splice(i,1); continue; }
        }
      }
      // 점막 위 = 둔화
      if(h.t>=h.warnT && player.y>yTop && player.y<yBot && (h.dir>0?player.x<h.front:player.x>h.front)) player._creepSlow=true;
      // 방치 페널티: 점막이 많이 깔리면 혜철이가 바이오매스로 회복 → 밀어낼 이유
      if(h.t>=h.warnT && h.phase!=='recede'){
        const cov=Math.abs(h.front-h.edge)/(W-80);
        if(cov>0.35){ const hc=enemies.find(o=>o.type==='hyechul'); if(hc&&hc.hp<hc.maxhp){ hc.hp=Math.min(hc.maxhp,hc.hp+cov*0.011*hc.maxhp*dt); if(Math.random()<0.25) burst(hc.x,hc.y,'#9a4fc0',2,130); } }
      }
      // 혜철이 없으면 즉시 후퇴 / 수명 종료 임박 시 후퇴
      if(!enemies.some(o=>o.type==='hyechul')) h.phase='recede';
      else if(h.t>=h.life-2.5 && h.phase==='advance') h.phase='recede';
      if(h.t>=h.life){ hazards.splice(i,1); }
      continue;
    }
    if(h.kind==='movelock'){
...
function drawHazards(){
  for(const h of hazards){
    ...
    if(h.kind==='megabeam'){
      const warn=h.t<h.warnT, k=warn?clamp(h.t/h.warnT,0,1):1;
      const activeT=Math.max(0,h.t-h.warnT), ang=h.ang+activeT*(h.rot||0);
      const len=h.len, w=h.width, col=h.col||'#58d8ff';
      const fade=warn?1:clamp(1-activeT/(h.liveT||1),0,1);
      ctx.save(); ctx.translate(h.x,h.y); ctx.rotate(ang);
      if(warn){
        ctx.globalAlpha=0.30+0.30*Math.abs(Math.sin(h.t*16));
        ctx.fillStyle='rgba(88,216,255,0.16)'; ctx.fillRect(-len,-w*k,len*2,w*2*k);
        ctx.globalAlpha=0.7; ctx.strokeStyle=col; ctx.lineWidth=2; ctx.setLineDash([14,10]);
        ctx.strokeRect(-len,-w,len*2,w*2); ctx.setLineDash([]);
      } else {
        const grid=Math.max(8,w*0.42), s=Math.ceil(grid);
        for(let y=-w;y<=w;y+=grid){ const edge=Math.abs(y)>w*0.62;
          for(let x=-len;x<=len;x+=grid){
            const sc=((Math.round(x/grid)+Math.round(performance.now()/60))%3===0);
            ctx.globalAlpha=fade*(edge?0.55:0.95);
            ctx.fillStyle=edge?_shade(col,-0.35):(sc?'#eafaff':col);
            ctx.fillRect(Math.round(x-grid/2),Math.round(y-grid/2),s,s);
          } }
        ctx.globalAlpha=fade; ctx.fillStyle='#ffffff'; ctx.fillRect(-len,-3,len*2,6);
      }
      ctx.restore();
      continue;
    }
    if(h.kind==='genpillar'){
      const warn=h.t<h.warnT;
      const activeT=Math.max(0,h.t-h.warnT), fade=warn?1:clamp(1-activeT/(h.liveT||0.55),0,1);
      const x=h.x, w=h.w, col='#bfe9ff';
      ctx.save();
      if(warn){
        ctx.globalAlpha=0.25+0.3*Math.abs(Math.sin(h.t*18));
        ctx.fillStyle='rgba(191,233,255,0.14)'; ctx.fillRect(x-w/2,0,w,H);
        ctx.strokeStyle=col; ctx.lineWidth=2; ctx.setLineDash([12,9]);
        ctx.beginPath(); ctx.moveTo(x-w/2,0); ctx.lineTo(x-w/2,H); ctx.moveTo(x+w/2,0); ctx.lineTo(x+w/2,H); ctx.stroke(); ctx.setLineDash([]);
      } else {
        const grid=Math.max(8,w*0.22), s=Math.ceil(grid);
        for(let y=0;y<H;y+=grid){
          for(let xx=x-w/2;xx<=x+w/2;xx+=grid){
            const edge=Math.abs(xx-x)>w*0.32;
            const stream=((Math.round(y/grid)+Math.round(performance.now()/40))%4===0);
            ctx.globalAlpha=fade*(edge?0.5:0.92);
            ctx.fillStyle=edge?_shade(col,-0.3):(stream?'#ffffff':col);
            ctx.fillRect(Math.round(xx-grid/2),Math.round(y),s,s);
          } }
        ctx.globalAlpha=fade*0.9; ctx.fillStyle='#fff'; ctx.fillRect(x-2,0,4,H);
      }
      ctx.restore();
      continue;
    }
    ...
    if(h.kind==='creepTide'){
      const yTop=100, yBot=H-50, hh=yBot-yTop, warn=h.t<h.warnT;
      ctx.save();
      if(warn){
        const k=h.t/h.warnT, bw=84, bx0=h.dir>0?0:W-bw;
        const gg=ctx.createLinearGradient(h.dir>0?0:W,0,h.dir>0?bw:W-bw,0);
        gg.addColorStop(0,'rgba(150,79,192,'+(0.30*(0.5+0.5*Math.abs(Math.sin(h.t*14))))+')');
        gg.addColorStop(1,'rgba(150,79,192,0)');
        ctx.fillStyle=gg; ctx.fillRect(bx0,yTop,bw,hh);
        ctx.globalAlpha=0.5+0.3*Math.abs(Math.sin(h.t*14));
        ctx.strokeStyle='#c46bff'; ctx.lineWidth=2.5; ctx.setLineDash([10,8]);
        const ex=h.dir>0?bw*k:W-bw*k; ctx.beginPath(); ctx.moveTo(ex,yTop); ctx.lineTo(ex,yBot); ctx.stroke(); ctx.setLineDash([]);
        ctx.restore(); continue;
      }
      const edge=h.edge, front=h.front, w=Math.max(1,Math.abs(front-edge)), seg=20, tt=h.t;
      // 물결치는 유기적 선단(점막 혀)
      const frontX=(yy)=>front - h.dir*(6 + 7*Math.sin(yy*0.035+tt*2.2) + 4*Math.sin(yy*0.09-tt*1.4));
      // 본체: 벽쪽 짙음 → 선단쪽 발광 그라데이션
      const g=ctx.createLinearGradient(edge,0,front,0);
      g.addColorStop(0,'rgba(28,14,42,0.64)'); g.addColorStop(0.55,'rgba(74,32,104,0.56)'); g.addColorStop(1,'rgba(150,64,190,0.5)');
      ctx.beginPath(); ctx.moveTo(edge,yTop); ctx.lineTo(edge,yBot);
      for(let s=seg;s>=0;s--){ const yy=yTop+s/seg*hh; ctx.lineTo(frontX(yy),yy); }
      ctx.closePath(); ctx.fillStyle=g; ctx.fill();
      // 정맥(벽→선단으로 뻗는 줄기)
      ctx.globalAlpha=0.38; ctx.strokeStyle='#7a3f9f'; ctx.lineWidth=2;
      for(let v=0;v<5;v++){ const vy=yTop+(v+0.5)/5*hh+Math.sin(tt*0.8+v)*10, fx=frontX(vy);
        ctx.beginPath(); ctx.moveTo(edge,vy);
        ctx.bezierCurveTo(edge+(fx-edge)*0.4,vy+18*Math.sin(v+tt),edge+(fx-edge)*0.7,vy-14*Math.cos(v*1.3+tt),fx,vy); ctx.stroke(); }
      // 농포(끓는 방울) — 짙은 베이스 + 밝은 하이라이트
      for(let k=0;k<10;k++){ const ka=h.seed+k*1.27+tt*0.9;
        const bx=edge+h.dir*((Math.sin(ka)*0.5+0.5)*w*0.92), by=yTop+(Math.cos(ka*1.7)*0.5+0.5)*hh, br=4+3*Math.sin(tt*2.5+k);
        ctx.globalAlpha=0.55; ctx.fillStyle='#3a1c52'; ctx.beginPath(); ctx.arc(bx,by,br+1.5,0,TAU); ctx.fill();
        ctx.globalAlpha=0.7; ctx.fillStyle='#b06bd8'; ctx.beginPath(); ctx.arc(bx-br*0.3,by-br*0.3,br*0.6,0,TAU); ctx.fill(); }
      ctx.globalAlpha=1;
      // 발광 막(선단 물결 라인 — 쏘는 가장자리)
      ctx.shadowColor='#d68bff'; ctx.shadowBlur=16; ctx.globalAlpha=0.85+0.15*Math.sin(tt*6);
      ctx.strokeStyle='#e6a8ff'; ctx.lineWidth=3.5; ctx.beginPath();
      for(let s=0;s<=seg;s++){ const yy=yTop+s/seg*hh, fx=frontX(yy); s===0?ctx.moveTo(fx,yy):ctx.lineTo(fx,yy); }
      ctx.stroke();
      // 내측 보조 라인(두께감)
      ctx.shadowBlur=0; ctx.globalAlpha=0.4; ctx.strokeStyle='#7a3f9f'; ctx.lineWidth=1.5; ctx.beginPath();
      for(let s=0;s<=seg;s++){ const yy=yTop+s/seg*hh, fx=frontX(yy)-h.dir*6; s===0?ctx.moveTo(fx,yy):ctx.lineTo(fx,yy); }
      ctx.stroke();
      ctx.restore(); continue;
    }
    if(h.kind==='movelock'){
...
function drawBossEvolve(){
  ...
  if(ba>0){
    ...
    ctx.textAlign='center'; ctx.fillStyle=E.col; ctx.font='bold 13px sans-serif'; const _evWho=(E.e&&(E.e.label||E.e.name))||''; ctx.fillText('🧬 '+_evWho, W/2, by+18);
...
function drawKijoFx(){
  ...
  // === 무대 암전(스포트라이트) — 빛=위험, 어둠=안전 ===
  if(boss&&boss.key==='kijo'&&(boss.stageT||0)>0){
    ctx.save();
    ctx.globalAlpha=0.62; ctx.fillStyle='#08040e'; ctx.fillRect(0,0,W,H);
    for(const s of (boss.stageSpots||[])){
      const g=ctx.createRadialGradient(s.x,s.y,8,s.x,s.y,s.r);
      g.addColorStop(0,'rgba(255,224,150,0.40)'); g.addColorStop(0.7,'rgba(255,180,110,0.18)'); g.addColorStop(1,'rgba(255,150,90,0)');
      ctx.globalAlpha=1; ctx.globalCompositeOperation='lighter'; ctx.fillStyle=g; ctx.beginPath(); ctx.arc(s.x,s.y,s.r,0,TAU); ctx.fill();
      ctx.globalCompositeOperation='source-over';
      ctx.globalAlpha=0.5+0.3*Math.sin(performance.now()/120); ctx.strokeStyle='#ff5a5a'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(s.x,s.y,s.r,0,TAU); ctx.stroke();
    }
    ctx.restore();
  }
  // === 보지 마(시선) — 키죠를 조준하면 피해 ===
  if(boss&&boss.key==='kijo'&&(boss.gazeT||0)>0){
    const charge=clamp(boss.gazeCharge||0,0,1), armed=charge>=1;
    const off=(boss.gazeAimOff==null?9:boss.gazeAimOff), warnBand=0.85;
    const near=clamp((warnBand-off)/(warnBand-0.46),0,1);   // 콘에 얼마나 가까운지(1=콘 안)
    const hot=Math.max(near*0.6,charge);
    ctx.save();
    // 시선 줄기 (가까울수록/충전될수록 붉고 굵게)
    ctx.globalAlpha=0.28+0.4*hot; ctx.strokeStyle=armed?'#ff2d4d':(hot>0.3?'#ff8a4d':'#c46bff'); ctx.lineWidth=2+3*hot; ctx.setLineDash([8,7]);
    ctx.beginPath(); ctx.moveTo(boss.x,boss.y); ctx.lineTo(player.x,player.y); ctx.stroke(); ctx.setLineDash([]);
    // 키죠 거대 눈
    const er=boss.r+16+3*Math.sin(performance.now()/110);
    ctx.globalAlpha=0.6; ctx.strokeStyle=armed?'#ff2d4d':'#d68bff'; ctx.lineWidth=3;
    ctx.beginPath(); ctx.arc(boss.x,boss.y,er,0,TAU); ctx.stroke();
    ctx.globalAlpha=0.85; ctx.fillStyle=armed?'#ff2d4d':'#c46bff';
    ctx.beginPath(); ctx.arc(boss.x,boss.y,7,0,TAU); ctx.fill();
    // === 커서 경고 ===
    if(near>0.02||charge>0){
      const mx=mouseX, my=mouseY, warn=Math.max(near,charge);
      const col=armed?'#ff2d4d':(charge>0?'#ff5a2a':'#ffd34d');
      const rr=Math.max(9,26-12*warn+2*Math.sin(performance.now()/70));
      ctx.globalAlpha=0.5+0.4*warn; ctx.strokeStyle=col; ctx.lineWidth=2.5;
      ctx.beginPath(); ctx.arc(mx,my,rr,0,TAU); ctx.stroke();
      // 충전 게이지 호 (콘 안에서 충전 중)
      if(charge>0&&!armed){ ctx.globalAlpha=0.95; ctx.strokeStyle='#ff2d4d'; ctx.lineWidth=3.5; ctx.beginPath(); ctx.arc(mx,my,rr,-Math.PI/2,-Math.PI/2+charge*TAU); ctx.stroke(); }
      ctx.globalAlpha=0.85+0.15*Math.sin(performance.now()/80); ctx.fillStyle=col; ctx.textAlign='center';
      ctx.font='bold 16px sans-serif'; ctx.fillText('👁', mx, my-rr-8);
      if(near>0.5||charge>0){ ctx.font='bold 11px sans-serif'; ctx.fillText(armed?'보지 마!':'시선 피해!', mx, my+rr+15); }
      ctx.textAlign='left';
    }
    // 피해 중 화면 비네트
    if(armed){
      const vg=ctx.createRadialGradient(W/2,H/2,Math.min(W,H)*0.3,W/2,H/2,Math.max(W,H)*0.62);
      vg.addColorStop(0,'rgba(255,40,60,0)'); vg.addColorStop(1,'rgba(255,40,60,'+(0.22+0.12*Math.sin(performance.now()/70))+')');
...
const BULLET_PATTERNS={
  ...
  breath: [".1.1.","13231","32223","13231",".1.1."],          // 온스터 날숨(거친 숨결 덩어리)
  genesis:["..3..",".323.","31213","32123","..3.."],           // 제네시스 빛 파편
};
const DIR_KINDS={chain:1,needle:1,silk:1,bolt:1};
const KIND_COL={ spore:'#7be04a', chain:'#8d72ff', mask:'#ff5a4a', energy:'#ff4dd2', needle:'#cfe0ff', silk:'#7ed957', glitch:'#38e8ff', bolt:'#ffd34d', jelly:'#ff7ad2', orb:'#9b6bff', shard:'#38e8ff', breath:'#a98bff', genesis:'#bfe9ff' };
...
function draw(){
  ...
  if(state==='play' && typeof GL!=='undefined' && GL.keyRev>0) drawKeyRevOverlay();
  drawA3Screen();
...
const EndingCredits=(function(){
  ...
  const ACTS=[
    [ // 1막 · 봉식월드 1구역 고블린 소굴
      ...
      {n:'킬조이 (여 · 26세)',d:'2022년 5월 1일',p:'1구역 고블린 소굴',r:'흥을 깬 뒤로 소식이 끊김',t:'mob',tint:'#38e8ff',f:{hair:'bob',brow:0,eye:'wide',mouth:'smile',skin:0}},
    ...
    [ // 2막 · 봉식월드 2구역 광천김 소굴
      ...
      {n:'저격러 (불명)',d:'2021년 2월 14일',p:'2구역 망루',r:'도네 저격 직후 연결이 끊김',t:'mob',f:{hair:'cap',brow:2,eye:'slit',mouth:'flat',skin:0}},
  ...
  function creditsHtml(){
    ...
    return '<h2>STAFF</h2>'+
      ...
      '<div class="ec-cr"><span class="ec-lab">베타 테스터</span><br><b>자잘자 · 케터 · 키죠 · 노잭 · 러라 · 킬조이 · 양갱 · 타포</b></div>'+
...
window.a3test=function(name){
  ...
  const map={
    ...
    // ── 3막 v3 신규 패턴 ──
    claim:       ()=>act3TruckClaim(b),
    newswall:    ()=>act3TruckNewsWall(b),
    chainanchor: ()=>onsterChainAnchor(b),
...
const DMG_NUM_MAX=40;
const DMG_NUM_LIGHT_MAX=18;
const DMG_NUM_VERY_HEAVY=30;
const DMG_NUM_DRAW_MAX=32;
const DMG_TEXT_CACHE_MAX=160;
let dmgTextCache=Object.create(null);
let dmgTextCacheKeys=[];
...
function dmgKindVisual(kind,crit,tier){
  if(kind==='burn') return {col:'#ff8c3a',prefix:'🔥',glow:'rgba(255,90,20,0.55)'};
  if(kind==='poison') return {col:'#3dff8a',prefix:'☠',glow:'rgba(40,255,120,0.5)'};
  if(kind==='heal') return {col:'#5dff9b',prefix:'+',glow:'rgba(93,255,155,0.55)'};
  if(crit) return {col:dmgComboColor(Math.max(tier,0.34)),prefix:'⚡',glow:'rgba(255,180,40,0.65)'};
  return {col:dmgComboColor(tier),prefix:'',glow:'rgba(255,120,40,'+(0.22+tier*0.4).toFixed(2)+')'};
}
function trimDmgTextCache(){
  while(dmgTextCacheKeys.length>DMG_TEXT_CACHE_MAX){
    const key=dmgTextCacheKeys.shift();
    if(key) delete dmgTextCache[key];
  }
}
function getDmgTextSprite(txt,fontPx,col,glow,stroke,glowOn){
  const key=txt+'|'+fontPx+'|'+col+'|'+stroke+'|'+(glowOn?glow:'');
  let img=dmgTextCache[key];
  if(img) return img;
  const c=document.createElement('canvas');
  const g=c.getContext('2d');
  const pad=Math.ceil(fontPx*0.55);
  g.font='bold '+fontPx+'px Galmuri11, "Courier New", monospace';
  const w=Math.ceil(g.measureText(txt).width+pad*2+6);
  const h=Math.ceil(fontPx*1.45+pad*2);
  ...
  if(stroke){
    ...
    g.strokeStyle='rgba(8,4,16,0.9)';
    g.strokeText(txt,x,y);
  }
  if(glowOn){
    g.shadowColor=glow;
    g.shadowBlur=Math.max(4,Math.round(fontPx*0.35));
  }
  g.fillStyle=col;
  g.fillText(txt,x,y);
  g.shadowBlur=0;
  dmgTextCache[key]=c;
  dmgTextCacheKeys.push(key);
  trimDmgTextCache();
  return c;
}
function mergeDmgInto(existing,amt,crit,direct){
  existing.amt+=amt;
  existing.t=0;
  existing.max=Math.max(existing.max,crit?0.8:0.62);
  existing.vy=Math.min(existing.vy,crit?-50:-38);
  if(direct){
    existing.crit=existing.crit||crit;
    existing.combo=dmgCombo;
    existing.tier=clamp((dmgCombo-1)/DMG_COMBO_MAX,0,1);
  }
}
function findDmgMergeTarget(x,y,k,direct,windowT,range){
  let best=null, bestD2=Infinity;
  for(let i=dmgNums.length-1;i>=0;i--){
    const d=dmgNums[i];
    if(!d||d.t>windowT) continue;
    const sameGroup=direct ? (d.kind==='normal'||d.kind==='crit') : (d.kind===k);
    if(!sameGroup) continue;
    const dx=d.x-x, dy=d.y-y, d2=dx*dx+dy*dy;
    if(d2<range*range && d2<bestD2){ best=d; bestD2=d2; }
  }
  return best;
}
function makeRoomForDmgNum(important){
  if(dmgNums.length<DMG_NUM_MAX) return true;
  if(!important) return false;
  let idx=-1;
  for(let i=0;i<dmgNums.length;i++){
    const d=dmgNums[i];
    if(d && !d.crit && d.kind!=='heal'){ idx=i; break; }
  }
  dmgNums.splice(idx>=0?idx:0,1);
  return true;
}
function spawnDmgNum(x,y,amt,crit,kind){
  amt=Math.max(1,Math.round(Number(amt)||0));
  if(amt<=0) return;
  const k=kind||'normal';
  const direct=(k==='normal'||k==='crit'||crit);   // 직접 타격(콤보 집계 대상)
  if(direct){ dmgCombo++; dmgComboT=DMG_COMBO_WINDOW; }

  // 상태이상/연타 폭주 시 가까운 숫자를 적극 병합해서 생성량을 제한한다.
  const crowded=dmgNums.length>=DMG_NUM_LIGHT_MAX;
  const mergeT=direct ? (crowded?0.28:0.16) : (crowded?0.75:0.34);
  const mergeR=direct ? (crowded?56:32) : (crowded?140:58);
  const target=findDmgMergeTarget(x,y,k,direct,mergeT,mergeR);
  if(target){ mergeDmgInto(target,amt,crit,direct); return; }

  const important=!!crit || k==='heal' || (direct && amt>=10);
  if(!makeRoomForDmgNum(important)) return;
...
function updateDmgNums(dt){
  ...
  for(let i=dmgNums.length-1;i>=0;i--){
    const d=dmgNums[i];
    d.t+=dt; d.y+=d.vy*dt; d.vy*=0.90;
    if(d.t>=d.max) dmgNums.splice(i,1);
  }
}
function drawDmgNums(){
  if(!dmgNums.length) return;
  const fpsLow=(typeof _fps==='number' && _fps>0 && _fps<55);
  const heavy=dmgNums.length>DMG_NUM_LIGHT_MAX || fpsLow;
  const veryHeavy=dmgNums.length>DMG_NUM_VERY_HEAVY || (typeof _fps==='number' && _fps>0 && _fps<50);
  const drawMax=veryHeavy?24:(heavy?DMG_NUM_DRAW_MAX:DMG_NUM_MAX);
  let drawn=0;
  ctx.save();
  for(let i=dmgNums.length-1;i>=0;i--){
    const d=dmgNums[i];
    const important=!!d.crit || d.kind==='heal' || ((d.kind==='normal'||d.kind==='crit') && d.amt>=20);
    if(drawn>=drawMax && !important) continue;
    const life=d.t/d.max;
    const a=clamp(1-Math.pow(life,2.4),0,1);
    if(a<=0.02) continue;
    const popK=(heavy?1:(d.t<0.1 ? (1.5-0.5*(d.t/0.1)) : 1));
    const tier=d.tier||0;
    let base=d.crit?26:17;
    base*=(1+(heavy?Math.min(tier,0.45):tier)*0.7);
    const rawFontPx=Math.max(10,Math.round(base*popK));
    const fontPx=heavy?Math.round(rawFontPx/2)*2:rawFontPx;
    const v=dmgKindVisual(d.kind,d.crit,tier);
    const txt=v.prefix+d.amt;
    const stroke=!veryHeavy;
    const glowOn=!heavy;
    const img=getDmgTextSprite(txt,fontPx,v.col,v.glow,stroke,glowOn);
    ctx.globalAlpha=a;
    ctx.drawImage(img,Math.round(d.x-img.width/2),Math.round(d.y-img.height/2));
    drawn++;
  }
  ctx.globalAlpha=1;
  ctx.restore();
