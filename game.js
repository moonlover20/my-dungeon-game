...
function playerMoveSpeed(p){
  ...
  return Math.max(1,base*statMulFromBonus(Number(p&&p.moveSpeedAdd)||0,0.1));
}
...
const ACT_POOLS=[
  ...
  { normal:["gwangcheon_gim","reura","namu","ketter","pobear","blackstar","killjoy","apple"], elite:["kkotchung"] },
...
function projectileCloseMul(target,bullet,bossTarget){
  ...
  const close=dist2(sx,sy,target.x,target.y)<=160*160;
  if(!close) return 1;
...
function killEnemy(e){
  ...
  if(!isSummon){ let coin=irand(3,7); if(e.elite) coin+=irand(10,18); coin=Math.round(coin*statMulFromBonus(statBonusFromMul(player.goldMul),0));
...
function update(dt){
  ...
  if(player.dodgeCharges<player.dodgeMaxCharges){ if(player.dodgeCd>0) player.dodgeCd-=dt; if(player.dodgeCd<=0){ player.dodgeCharges++; if(player.dodgeCharges<player.dodgeMaxCharges) player.dodgeCd=10*playerDodgeCooldownMul(player); updateHUD(); } }
...
const EVENTS=[
  ...
  {tag:'💎 탐욕의 신전',title:'황금빛 신전',body:'신전은 생명과 돈을 같은 무게로 잰다. 어느 쪽을 바칠지 정해야 한다.',
   choices:[
     {t:'체력 20% 지불 → 골드 300',f:()=>{eventHpCostPct(0.20);gold+=300;try{sfx.coin&&sfx.coin();}catch(e){}banner('탐욕의 축복','골드 +300',1600);updateHUD();finishNode();}},
...
function spEffects(p){
  ...
  add(p.glassCannon||hasRelic('glass'),'🍷','유리 대포','유리 대포','공격력 대폭 증가, 최대 체력/받는 피해 리스크','glass-cannon');
